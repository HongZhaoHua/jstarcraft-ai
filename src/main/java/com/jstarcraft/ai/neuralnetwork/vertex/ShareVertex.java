...
public class ShareVertex extends LayerVertex {
	...
	public ShareVertex(String name, MatrixFactory factory, int numberOfShares, Layer layer) {
		this(name, factory, numberOfShares, layer, new IgnoreLearner(), new IgnoreNormalizer());
	}

	public ShareVertex(String name, MatrixFactory factory, int numberOfShares, Layer layer, Learner learner, Normalizer normalizer) {
		...
		for (Entry<String, MathMatrix> term : vertexGradients.entrySet()) {
			...
			matrix = factory.makeCache(matrix.getRowSize(), matrix.getColumnSize());
