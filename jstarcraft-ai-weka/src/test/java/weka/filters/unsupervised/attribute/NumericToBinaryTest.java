...
/**
 * Tests NumericToBinary. Run from the command line with:
 * <p>
...
public class NumericToBinaryTest extends AbstractFilterTest {
    ...
    public void testInvertSelection() {
        int att1 = m_Instances.attribute("NumericAtt1").index();
        int att2 = m_Instances.attribute("NumericAtt2").index();
        // Set the attribute index to point to NumericAtt1, but invert the selection, so
        // that
        // it will try to apply the filter to *every other* attribute. Of course,
        // because this
    ...
    /**
     * Make sure the filter binarizes the appropriate attributes in the range we
     * specify
     */
