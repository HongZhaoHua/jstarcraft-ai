...
/**
 * Tests AddExpression. Run from the command line with:
 * <p>
...
public class AddExpressionTest extends AbstractFilterTest {
    ...
    public void testAdd() {
        ...
        for (int i = 0; i < result.numInstances(); i++) {
            ...
            assertEquals("Instance " + (i + 1), inst.value(2) + inst.value(5), inst.value(inst.numAttributes() - 1), EXPR_DELTA);
        }
    }
    ...
    public void testBODMAS() {
        ...
        for (int i = 0; i < result.numInstances(); i++) {
            ...
            if (inst.isMissing(2) || inst.isMissing(5)) {
                ...
                assertEquals("Instance " + (i + 1), inst.value(2) + (inst.value(2) * inst.value(5)) - (inst.value(2) / 5) + (inst.value(2) * inst.value(5)) + inst.value(5), inst.value(inst.numAttributes() - 1), EXPR_DELTA);
            }
        }
    }
