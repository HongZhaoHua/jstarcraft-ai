...
public abstract class AbstractFilterTest extends TestCase {
    ...
    /**
     * Configures the CheckGOE used for testing GOE stuff. Sets the Filter returned
     * from the getFilter() method.
     * 
    ...
    /**
     * Used to create an instance of a specific filter. The filter should be
     * configured to operate on a dataset that contains attributes in this order:
     * <p>
     *
     * String, Nominal, Numeric, String, Nominal, Numeric
     * <p>
     *
     * Where the first three attributes do not contain any missing values, but the
     * last three attributes do. If the filter is for some reason incapable of
     * accepting a dataset of this type, override setUp() to either manipulate the
     * default dataset to be compatible, or load another test dataset.
     * <p>
     *
     * The configured filter should preferrably do something meaningful, since the
     * results of filtering are used as the default regression output (and it would
     * hardly be interesting if the filtered data was the same as the input data).
     *
    ...
    /**
     * Simple method to return the filtered set of test instances after passing
     * through the test filter. m_Filter contains the filter and m_Instances
     * contains the test instances.
     *
     * @return the Instances after filtering through the filter we have set up to
     *         test.
     */
    ...
    /**
     * Test buffered operation. Output instances are only collected after all
    ...
    /**
     * Test incremental operation. Each instance is removed as soon as it is made
     * available
     */
