...
public class Apriori extends AbstractAssociator implements OptionHandler, AssociationRulesProducer, CARuleMiner, TechnicalInformationHandler {
    ...
    /** Metric types. */
    public static final Tag[] TAGS_SELECTION = { new Tag(CONFIDENCE, "Confidence"), new Tag(LIFT, "Lift"), new Tag(LEVERAGE, "Leverage"), new Tag(CONVICTION, "Conviction") };
    ...
    /**
     * Constructor that allows to sets default values for the minimum confidence and
     * the maximum number of rules the minimum confidence.
     */
    public Apriori() {
    ...
    public void buildAssociations(Instances instances) throws Exception {
        ...
        do {
             ...
             * (double)((AprioriItemSet)m_allTheRules[1].elementAt(i)).support(); indices =
             * Utils.stableSort(supports); for (int i = 0; i < m_allTheRules[2].size(); i++)
             * { sortedRuleSet[0].add(m_allTheRules[0].get(indices[i]));
             * sortedRuleSet[1].add(m_allTheRules[1].get(indices[i]));
             * sortedRuleSet[2].add(m_allTheRules[2].get(indices[i])); if (m_metricType !=
             * CONFIDENCE || m_significanceLevel != -1) {
    ...
    /**
     * Method that mines all class association rules with minimum support and with a
     * minimum confidence.
     * 
    ...
    public String toString() {
        ...
        if (!m_car) {
            ...
            for (int i = 0; i < m_allTheRules[0].size(); i++) {
                /*
                 * text.append(Utils.doubleToString((double) i + 1, (int) (Math.log(m_numRules)
                 * / Math.log(10) + 1), 0) + ". " + ((AprioriItemSet) m_allTheRules[0].get(i))
                 * .toString(m_instances) + " ==> " + ((AprioriItemSet) m_allTheRules[1].get(i))
                 * .toString(m_instances)); text.append("    " + ((m_metricType == CONFIDENCE) ?
                 ...
                 * Utils.doubleToString( ((Double) m_allTheRules[3].get(i)).doubleValue(), 2) +
                 * ")" + (m_metricType == LIFT ? ">" : "")); text.append((m_metricType ==
                 * LEVERAGE ? " <" : "") + " lev:(" + Utils.doubleToString( ((Double)
                 * m_allTheRules[4].get(i)).doubleValue(), 2) + ")"); text.append(" [" + (int)
                 * (((Double) m_allTheRules[4].get(i)).doubleValue() * m_instances
                 * .numInstances()) + "]" + (m_metricType == LEVERAGE ? ">" : ""));
                 * text.append((m_metricType == CONVICTION ? " <" : "") + " conv:(" +
                 * Utils.doubleToString( ((Double) m_allTheRules[5].get(i)).doubleValue(), 2) +
