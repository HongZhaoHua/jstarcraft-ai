...
public class PART extends AbstractClassifier implements OptionHandler, WeightedInstancesHandler, Summarizable, AdditionalMeasureProducer, TechnicalInformationHandler {
    ...
    public void buildClassifier(Instances instances) throws Exception {
        ...
        if ((!m_reducedErrorPruning) && (m_numFolds != 3)) {
            throw new Exception("Changing the number of folds does not make sense if" + " reduced error pruning is not selected.");
        }
        if ((m_CF <= 0) || (m_CF >= 1)) {
    ...
    public String[] getOptions() {
        ...
        if (m_unpruned || !m_reducedErrorPruning) {
            ...
            if (m_numFolds != 3) {
                System.err.println("WARNING: Changing the number of folds does not make sense if " + "reduced error pruning is not selected.");
            }
        }
