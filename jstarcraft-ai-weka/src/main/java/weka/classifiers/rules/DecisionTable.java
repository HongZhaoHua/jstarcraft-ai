...
public class DecisionTable extends AbstractClassifier implements OptionHandler, WeightedInstancesHandler, AdditionalMeasureProducer, TechnicalInformationHandler {
    ...
    public static final Tag[] TAGS_EVALUATION = { new Tag(EVAL_DEFAULT, "Default: accuracy (discrete class); RMSE (numeric class)"), new Tag(EVAL_ACCURACY, "Accuracy (discrete class only"), new Tag(EVAL_RMSE, "RMSE (of the class probabilities for discrete class)"), new Tag(EVAL_MAE, "MAE (of the class probabilities for discrete class)"), new Tag(EVAL_AUC, "AUC (area under the ROC curve - discrete class only)") };
    ...
    /**
     * Classifies an instance for internal leave one out cross validation of feature
     * sets
     * 
    ...
    protected double evaluateInstanceLeaveOneOut(Instance instance, double[] instA) throws Exception {
        ...
        if (m_classIsNominal) {
            ...
            if ((tempDist = m_entries.get(thekey)) == null) {
                ...
                /*
                 * } else { normDist = new double [normDist.length]; normDist[(int)m_majority] =
                 * 1.0; if (m_evaluationMeasure == EVAL_AUC) {
                 * m_evaluation.evaluateModelOnceAndRecordPrediction(normDist, instance); } else
                 * { m_evaluation.evaluateModelOnce(normDist, instance); } return m_majority; }
                 */
            }
    ...
    protected double evaluateFoldCV(Instances fold, int[] fs) throws Exception {
        ...
        for (i = 0; i < numFold; i++) {
            ...
            if (m_classIsNominal) {
                 ...
                 * EVAL_AUC) { m_evaluation.evaluateModelOnceAndRecordPrediction(normDist,
                 * inst); } else { m_evaluation.evaluateModelOnce(normDist, inst); } }
                 */
            } else {
                if (Utils.eq(normDist[1], 0.0)) {
                    double[] temp = new double[1];
    ...
    /**
     * Sets up a dummy subset evaluator that basically just delegates evaluation to
     * the estimatePerformance method in DecisionTable
     */
