...
public class RuleStats implements Serializable, RevisionHandler {
     ...
     * RuleStats object, if the number set is smaller than 0 (typically -1), then it
     * calcualtes based on the data store
     * 
    ...
    /**
     * Compute the number of all possible conditions that could appear in a rule of
     * a given data. For nominal attributes, it's the number of values that could
     * appear; for numeric attributes, it's the number of values * 2, i.e. <= and >=
     ...
     * are not covered by the rules in position 0...(index-1), and the statistics of
     * these rules are provided.<br>
     ...
     * distributions are stored in the given double array, which can be obtained by
    ...
    /**
     * The description length of data given the parameters of the data based on the
     * ruleset.
     * <p>
    ...
    public static double dataDL(double expFPOverErr, double cover, double uncover, double fp, double fn) {
         ...
         * "|coverBits: "+coverBits+"|uncBits: "+ uncoverBits+ "|FPRate: "+expFPOverErr
         * + "|expErr: "+expErr+ "|fp: "+fp+"|fn: "+fn+"|total: "+totalBits);
         */
     ...
     * that could be decreased by deleting the rule whose index and simple statstics
     * are given. If there's no potentials (i.e. smOrEq 0 && error rate < 0.5), it
     * returns NaN.
     * <p>
     * 
     * The way this procedure does is copied from original RIPPER implementation and
     * is quite bizzare because it does not update the following rules' stats
     * recursively any more when testing each rule, which means it assumes after
     * deletion no data covered by the following rules (or regards the deleted rule
     * as the last rule). Reasonable assumption?
     * <p>
     * 
     * @param index        the index of the rule in m_Ruleset to be deleted
     * @param expFPOverErr expected FP/(FP+FN)
     * @param rulesetStat  the simple statistics of the ruleset, updated if the rule
     *                     should be deleted
    ...
    /**
     * Compute the minimal data description length of the ruleset if the rule in the
     * given position is deleted.<br>
    ...
    /**
     * Compute the minimal data description length of the ruleset if the rule in the
     * given position is NOT deleted.<br>
     ...
     * <p>
     * 
     * @param index     the given position of the rule in question (assuming
     *                  correct)
     * @param expFPRate expected FP/(FP+FN), used in dataDL calculation
     * @param checkErr  whether check if error rate >= 0.5
    ...
    /**
     * Try to reduce the DL of the ruleset by testing removing the rules one by one
     * in reverse order and update all the stats
     * 
     * @param expFPRate expected FP/(FP+FN), used in dataDL calculation
     * @param checkErr  whether check if error rate >= 0.5
     */
    ...
    /**
     * Remove the last rule in the ruleset as well as it's stats. It might be useful
     * when the last rule was added for testing purpose and then the test failed
     */
     ...
     * given index in the given rules, and then remove them. It returns the data not
     * covered by the successive rules.
     * 
     ...
     * before stratification it sorts the instances according to the class order in
     * the header file. It assumes no missing values in the class.
     * 
     * @param data  the given data
    ...
    /**
     * Compute the combined DL of the ruleset in this class, i.e. theory DL and data
     * DL. Note this procedure computes the combined DL according to the current
     * status of the ruleset in this class
     * 
     * @param expFPRate expected FP/(FP+FN), used in dataDL calculation
