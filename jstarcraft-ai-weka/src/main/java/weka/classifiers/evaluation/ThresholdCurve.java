...
public class ThresholdCurve implements RevisionHandler {
    ...
    /**
     * Calculates the performance stats for the default class and return results as
     * a set of Instances. The structure of these Instances is as follows:
     * <p>
     * <ul>
    ...
    /**
     * Calculates the performance stats for the desired class and return results as
     * a set of Instances.
     * 
     * @param predictions the predictions to base the curve on
    ...
    public Instances getCurve(ArrayList<Prediction> predictions, int classIndex) {
        ...
        for (int i = 0; i < sorted.length; i++) {
             ...
             * probs[sorted[i - 1]]))) { insts.add(makeInstance(tc, probs[sorted[i]])); }
             */
        }
    ...
    /**
     * Calculates the n point precision result, which is the precision averaged over
     * n evenly spaced (w.r.t recall) samples of the curve.
     * 
    ...
    public static double getNPointPrecision(Instances tcurve, int n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            /*
             * System.err.println("Point " + (i + 1) + ": i=" + pos + " r=" + (i * isize) +
             * " p'=" + precis + " r'=" + recall);
             */
            ...
            while ((pos != 0) && (pos < sorted.length - 1)) {
                ...
                if (recall2 != recall) {
                    ...
                    /*
                     * System.err.println("Point2 " + (i + 1) + ": i=" + pos + " r=" + (i * isize) +
                     * " p'=" + precis2 + " r'=" + recall2 + " p''=" + precis);
                     */
                    break;
                }
            }
    ...
    /**
     * Tests the ThresholdCurve generation from the command line. The classifier is
