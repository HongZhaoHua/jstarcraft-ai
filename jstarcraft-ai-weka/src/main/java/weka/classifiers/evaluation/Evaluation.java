 ...
 * Output models for training splits if cross-validation or percentage-split
 * evaluation is used.
 * <p/>
 *
...
public class Evaluation implements Summarizable, RevisionHandler, Serializable {
    ...
    /**
     * The minimum probability accepted from an estimator to avoid taking log(0) in
     * Sf calculations.
     */
     ...
     * <code>setPriors(Instances)</code>.
     *
     * @param data       set of training instances, to get some header information
     *                   and prior class distribution information
    ...
    /**
     * Set a list of the names of metrics to have appear in the output. The default
    ...
    /**
     * Returns the area under ROC for those predictions that have been collected in
     * the evaluateClassifier(Classifier, Instances) method. Returns
     ...
     * on a set of instances. Now performs a deep copy of the classifier before each
     ...
     *                         accepted by the classifier will be removed from this
     *                         array.
     * @throws Exception if a classifier could not be generated successfully or the
     *                   class is not defined
     */
     ...
     * <p>
     * -l filename <br/>
     * Loads classifier from the given file. In case the filename ends with ".xml",a
     * PMML file is loaded or, if that fails, options are loaded from XML.
     * <p/>
     * <p>
     * -d filename <br/>
     * Saves classifier built from the training data into the given file. In case
    ...
    /**
     * Saves the given classifier, along with the template Instances object (if
     * appropriate) to the given file.
     *
    ...
    public static String evaluateModel(Classifier classifier, String[] options) throws Exception {
        ...
        try {
            ...
            // Read model file if appropriate (which may just hold scheme-specific options,
            // and not a built model)
            if (objectInputFileName.length() != 0) {
        ...
        // Stop here if no output of performance statistics or predictions is required
        // and no threshold data is required
    ...
    /**
     * Evaluates the classifier on a given set of instances. Note that the data must
     * have exactly the same format (e.g. order of attributes) as the data used to
     * train the classifier! Otherwise the results will generally be meaningless.
     *
     * @param classifier             machine learning classifier
    ...
    /**
     * Gets the number of test instances that had a known class value (actually the
     * sum of the weights of test instances with known class value).
     *
    ...
    /**
     * Gets the average size of the predicted regions, relative to the range of the
     * target in the training data, at the confidence level specified when
    ...
    /**
     * Gets the total cost, that is, the cost of each prediction times the weight of
     * the instance, summed over all instances.
     *
    ...
    /**
     * Gets the number of instances not classified (that is, for which no prediction
     * was made by the classifier). (Actually the sum of the weights of these
     * instances)
     *
    ...
    /**
     * Returns the estimated error rate or the root mean squared error (if the class
     * is numeric). If a cost matrix was given this error rate gives the average
     * cost.
     *
    ...
    /**
     * Returns the mean absolute error. Refers to the error of the predicted values
     * for numeric classes, and the error of the predicted probability distribution
     * for nominal classes.
     *
    ...
    /**
     * Return the Kononenko & Bratko Relative Information score. Differs slightly
     * from the expression used in KB's paper because it uses the mean log-loss of
     * the TEST instances wrt to the null model for normalization.
     *
    ...
    /**
     * Output the cumulative margin distribution as a string suitable for input for
     * gnuplot or similar package.
     *
     ...
     *
     * @param printComplexityStatistics if true, complexity statistics are returned
     *                                  as well
     ...
     * percentage) of instances classified correctly, incorrectly and unclassified.
     * Outputs the total number of instances classified, and the number of instances
     * (if any) that had no class value provided.
     *
     ...
     * precision/recall/F-Measure. Should be useful for ROC curves, recall/precision
     * curves.
     *
     ...
     * This is equal to the sum of all entries in the confusion matrix.
     *
     ...
     * only includes instances for which a prediction is made. Unclassified
     * instances are not included even if they have a class value.
     *
     * @param classIndex the index of the class to consider as "positive"
    ...
    /**
     * Calculate the number (really, weight) of instances predicted to be of the
     * given class.
     *
     * Unclassified instances are not included in the calculation.
     *
     ...
     * This only includes instances for which a prediction is made. Unclassified
     * instances are not included even if they have a class value.
     *
     * @param classIndex the index of the class to consider as "positive"
    ...
    /**
     * Calculate the number (really, weight) of instances predicted not to be of the
     * given class.
     *
     * Unclassified instances are not included in the calculation.
     *
    ...
    /**
     * Returns the number (really, weight) of instances in each class. Unclassified
     ...
     * Unclassified instances are not included in the calculation.
     *
     * @return an array with the predicted number (really, weight) of instances in
     *         each class
     */
    ...
    /**
     * Calculate the number (really, weight) of true positives with respect to a
    ...
    /**
     * Calculate number (really, weight) of false negatives with respect to a
     * particular class. This is defined as
     * <p/>
     *
     * <pre>
    ...
    /**
     * Calculate the number (really, weight) of true negatives with respect to a
     * particular class. This is defined as
     * <p/>
     *
     * <pre>
    ...
    /**
     * Calculate the number (really, weight) of false positives with respect to a
     * particular class. This is defined as
     * <p/>
     *
     * <pre>
    ...
    /**
     * Calculate the true positive rate with respect to a particular class. This is
     * defined as
     * <p/>
     *
     * <pre>
     ...
     * </pre>
     *
     * This does not include any instances that are left unclassified by the
     * classifier.
     *
     * @param classIndex the index of the class to consider as "positive"
     * @return the true positive rate
     ...
     *
     * @param classIndex   the index of the class to consider as "positive"
     * @param numPositives the total number (really, weight) of positives for that
     *                     class
     * @return the true positive rate
     */
    ...
    /**
     * Calculates the weighted (by class size) true positive rate. Unclassified
     * instances are not included in the calculation.
     *
     * Unclassified instances are not included in the calculation.
     *
    ...
    /**
     * Calculate the true negative rate with respect to a particular class. This is
     * defined as
     * <p/>
     *
     * <pre>
    ...
    /**
     * Calculates the weighted (by class size) true negative rate. Unclassified
     * instances are not included in the calculation.
     *
     * Unclassified instances are not included in the calculation.
     *
    ...
    /**
     * Calculate the false positive rate with respect to a particular class. This is
     * defined as
     * <p/>
     *
     * <pre>
    ...
    /**
     * Calculates the weighted (by class size) false positive rate. Unclassified
     * instances are not included in the calculation.
     *
     * Unclassified instances are not included in the calculation.
     *
    ...
    /**
     * Calculate the false negative rate with respect to a particular class. This is
     * defined as
     * <p/>
     *
     * <pre>
     ...
     *
     * @param classIndex   the index of the class to consider as "positive"
     * @param numPositives the number (really, weight) of the instances in the
     *                     positive class
     * @return the false positive rate
     */
     ...
     * coefficient) for the supplied class. Unclassified instances are not included
     * in the calculation.
     *
     * @param classIndex the index of the class to compute the matthews correlation
     *                   coefficient for
     *
     *                   Unclassified instances are not included in the calculation.
     *
    ...
    public double matthewsCorrelationCoefficient(int classIndex) {
        ...
        return matthewsCorrelationCoefficient(classIndex, numTruePositives(classIndex), numTrueNegatives(classIndex), numFalsePositives(classIndex), numFalseNegatives(classIndex));
    }
    ...
    public double weightedMatthewsCorrelation() {
        ...
        for (int i = 0; i < m_NumClasses; i++) {
            if (trueClassCounts[i] > 0) { // If MCC is NaN, we want the sum to also be NaN if count > 0
                ...
                mccTotal += matthewsCorrelationCoefficient(i, numTruePositives(i), (numClassified() - trueClassCounts[i]) - numFP, numFP, trueClassCounts[i] - numTruePositives(i)) * trueClassCounts[i];
            }
        }
    ...
    public double weightedFMeasure() {
        ...
        for (int i = 0; i < m_NumClasses; i++) {
            if (trueClassCounts[i] > 0) { // If F-measure is NaN, we want the sum to also be NaN if count > 0
                fMeasureTotal += fMeasure(numTruePositives(i) / predictedClassCounts[i], numTruePositives(i) / trueClassCounts[i]) * trueClassCounts[i];
            }
        }
    ...
    /**
     * Unweighted macro-averaged F-measure. If some classes not present in the test
     * set, they're just skipped (since recall is undefined there anyway) .
     *
     * Unclassified instances are not included in the calculation.
     *
    ...
    public double unweightedMacroFmeasure() {
        ...
        for (int c = 0; c < m_NumClasses; c++) {
            ...
            if (trueClassCounts[c] > 0) {
                rr.add(fMeasure(numTruePositives(c) / predictedClassCounts[c], numTruePositives(c) / trueClassCounts[c]));
            }
        }
        rr.calculateDerived();
        return rr.mean;
    ...
    /**
     * Unweighted micro-averaged F-measure. If some classes not present in the test
     * set, they have no effect.
     *
    ...
    /**
     * disables the use of priors, e.g., in case of de-serialized schemes that have
     * no access to the original training set, but are evaluated on a set set.
     */
     ...
     * @param classMissing the instance for which density is to be computed, without
     *                     a class value
     * @param classValue   the class value of this instance
    ...
    /**
     * Adds a numeric (non-missing) training class value and weight to the buffer of
     * stored values. Also updates minimum and maximum target value.
     * 
