...
public class REPTree extends AbstractClassifier implements OptionHandler, WeightedInstancesHandler, Drawable, AdditionalMeasureProducer, Sourcable, PartitionGenerator, Randomizable {
    ...
    protected class Tree implements Serializable, RevisionHandler {
        ...
        /**
         * The (unnormalized) class distribution in the nominal case. Holds the sum of
         * squared errors and the weight in the numeric case.
         */
        protected double[] m_Distribution = null;

        /**
         * Class distribution of hold-out set at node in the nominal case. Straight sum
         * of weights plus sum of weighted targets in the numeric case (i.e. array has
         * only two elements).
         */
        protected double[] m_HoldOutDist = null;

        /**
         * The hold-out error of the node. The number of miss-classified instances in
         * the nominal case, the sum of squared errors in the numeric case.
         */
         ...
         * this node. The instance being tested is called "i". This routine assumes to
         * be called in the order of branching, enabling us to set the >= condition test
         * (the last one) of a numeric splitpoint to just "true" (because being there in
         * the flow implies that the previous less-than test failed).
         * 
        ...
        /**
         * Returns source code for the tree as if-then statements. The class is assigned
         * to variable "p", and assumes the tested instance is named "i". The results
         * are returned as two stringbuffers: a section of code for assignment of the
         * class, and a section of code containing support code (eg: other support
         * methods).
         * <p/>
         * TODO: If the outputted source code encounters a missing value for the
         * evaluated attribute, it stops branching and uses the class distribution of
         * the current node to decide the return value. This is unlike the behaviour of
        ...
        protected void buildTree(int[][][] sortedIndices, double[][][] weights, Instances data, double totalWeight, double[] classProbs, Instances header, double minNum, double minVariance, int depth, int maxDepth) throws Exception {
            ...
            // Nominal case
                    (data.classAttribute().isNominal() && Utils.eq(m_ClassProbs[Utils.maxIndex(m_ClassProbs)], Utils.sum(m_ClassProbs))) ||
    ...
    /**
     * The minimum proportion of the total variance (over all the data) required for
     * split.
     */
    ...
    /**
     * Computes array that indicates node membership. Array locations are allocated
     * based on breadth-first exploration of the tree.
     */
    @Override
