...
public class LogisticBase extends AbstractClassifier implements WeightedInstancesHandler {
    ...
    /**
     * Numeric version of the training data. Original class is replaced by a numeric
     * pseudo-class.
     */
    ...
    /**
     * Threshold for trimming weights. Instances with a weight lower than this (as a
     * percentage of total weights) are not included in the regression fit.
     **/
    ...
    /**
     * Runs LogitBoost, determining the best number of iterations by an information
     * criterion (currently AIC).
     */
    ...
    /**
     * Runs LogitBoost on a training set and monitors the error on a test set. Used
     * for running one fold when cross-validating the number of LogitBoost
     * iterations.
     * 
     ...
     *         maxIterations if the heuristic for early stopping is active or there
     *         is a problem while fitting the regressions in LogitBoost).
     * @throws Exception if something goes wrong
     */
    ...
    /**
     * Runs LogitBoost using the stopping criterion on the training set. The number
     * of iterations is used that gives the lowest error on the training set, either
     * misclassification or error on probabilities (depending on the
    ...
    /**
     * Returns the error of the probability estimates for the current model on a set
     * of instances.
     * 
     * @param data the set of instances
     * @return the error
     ...
     * A simple regression function is fit to the response and added to the
     * m_regressions array.
     * 
     * @param iteration    the current iteration
     * @param trainYs      the y-values (see description of LogitBoost) for the
     *                     model trained so far
     * @param trainFs      the F-values (see description of LogitBoost) for the
     *                     model trained so far
     * @param probs        the p-values (see description of LogitBoost) for the
     *                     model trained so far
     * @param trainNumeric numeric version of the training data
     * @return returns true if iteration performed successfully, false if no simple
     *         regression function could be fitted.
     * @throws Exception if something goes wrong
     */
    ...
    /**
     * Converts training data to numeric version. The class variable is replaced by
     * a pseudo-class used by LogitBoost.
     * 
    ...
    /**
     * Computes the LogitBoost response for an array of y/p values (actual/estimated
     * class probabilities).
     * 
    ...
    /**
     * Computes the LogitBoost weights from an array of y/p values (actual/estimated
     * class probabilities).
     * 
     * @param dataYs the actual class probabilities
     * @param probs  the estimated class probabilities
     ...
     * The first dimension is the class, the second dimension holds a list of
     * attribute indices. Attribute indices start at zero.
     * 
     ...
     * logistic model (in percent). An attribute is used in the model if it is used
     * in any of the models for the different classes.
     * 
