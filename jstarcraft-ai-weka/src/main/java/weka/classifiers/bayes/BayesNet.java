...
public class BayesNet extends AbstractClassifier implements OptionHandler, WeightedInstancesHandler, Drawable, AdditionalMeasureProducer {
     ...
     * "--" but before the "-E". These options are removed from the original options
     * array.
     * 
     ...
     * http://www-2.cs.cmu.edu/~fgcozman/Research/InterchangeFormat/ for details on
     * XML BIF.
     * 
    ...
    public String toXMLBIF03() {
        ...
        for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
            ...
            for (int iValue = 0; iValue < m_Instances.attribute(iAttribute).numValues(); iValue++) {
                text.append("<OUTCOME>" + XMLNormalize(Utils.quote(m_Instances.attribute(iAttribute).value(iValue))) + "</OUTCOME>\n");
            }
        ...
        for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
            ...
            for (int iParent = 0; iParent < m_ParentSets[iAttribute].getNrOfParents(); iParent++) {
                text.append("<GIVEN>" + XMLNormalize(Utils.quote(m_Instances.attribute(m_ParentSets[iAttribute].getParent(iParent)).name())) + "</GIVEN>\n");
            }
            text.append("<TABLE>\n");
