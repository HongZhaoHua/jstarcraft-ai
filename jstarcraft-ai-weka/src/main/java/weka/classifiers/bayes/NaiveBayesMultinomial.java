...
public class NaiveBayesMultinomial extends AbstractClassifier implements WeightedInstancesHandler, TechnicalInformationHandler {
    ...
    /**
     * probability that a word (w) exists in a class (H) (i.e. Pr[w|H]) The matrix
     * is in the this format: probOfWordGivenClass[class][wordAttribute] NOTE: the
     * values are actually the log of Pr[w|H]
     */
    ...
    public void buildClassifier(Instances instances) throws Exception {
        ...
        /*
         * normalising probOfWordGivenClass values and saving each value as the log of
         * each value
         */
        for (int c = 0; c < m_numClasses; c++) {
    ...
    public String toString() {
        ...
        for (int w = 0; w < m_numAttributes; w++) {
            if (w != m_headerInfo.classIndex()) {
                ...
                for (int c = 0; c < m_numClasses; c++)
                    result.append(Utils.doubleToString(Math.exp(m_probOfWordGivenClass[c][w]), getNumDecimalPlaces())).append("\t");
                result.append("\n");
            }
        }
