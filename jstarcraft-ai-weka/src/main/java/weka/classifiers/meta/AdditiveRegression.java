 ...
 * regression base classifier. Each iteration fits a model to the residuals left
 * by the classifier on the previous iteration. Prediction is accomplished by
 * adding the predictions of each classifier. Reducing the shrinkage (learning
 * rate) parameter helps prevent overfitting and has a smoothing effect but
...
public class AdditiveRegression extends IteratedSingleClassifierEnhancer implements OptionHandler, AdditionalMeasureProducer, WeightedInstancesHandler, TechnicalInformationHandler, IterativeClassifier {
    ...
    /**
     * ArrayList for storing the generated base classifiers. Note: we are hiding the
     * variable from IteratedSingleClassifierEnhancer
     */
     ...
     * residuals after predicting with the supplied classifier.
     *
    ...
    /**
     * Replace the class values of the instances from the current iteration with
     * residuals after predicting the given constant.
     *
     * @param data the instances to predict
