...
public class RegSMO extends RegOptimizer implements TechnicalInformationHandler {
     ...
     * note, we don't need m_b in the cache, since if we do, we need to maintain it
     * when m_b is updated
     */
    double[] m_error;
    ...
    /**
     * applies heuristic for finding candidate that is expected to lead to good gain
     * when applying takeStep together with second candidate.
     * 
    ...
    /**
     * learn SVM parameters from data using Smola's SMO algorithm. Subclasses should
     * implement something more interesting.
     * 
