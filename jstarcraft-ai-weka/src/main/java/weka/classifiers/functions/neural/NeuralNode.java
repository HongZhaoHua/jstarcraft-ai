...
public class NeuralNode extends NeuralConnection {
    ...
    /**
     * Performs the operations for this node. Currently this defines that the node
     * is either a sigmoid or a linear unit.
     */
    ...
    /**
     * Set how this node should operate (note that the neural method has no internal
     * state, so the same object can be used by any number of nodes.
     * 
     ...
     * @return The value for the specified connection or if -1 then it should return
     *         the threshold value. If no value exists for the specified connection,
    ...
    /**
     * call this function to get the weights array. This will also allow the weights
     * to be updated.
     * 
     ...
     * the change in weights to be updated.
     * 
     ...
     *          this unit should be severed (not the same as removeAllInputs).
