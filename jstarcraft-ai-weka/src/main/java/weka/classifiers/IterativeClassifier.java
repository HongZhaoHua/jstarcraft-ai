...
public interface IterativeClassifier extends Classifier {
     ...
     * potentially make serialized model structures larger, because certain data and
     * data structures might need to be retained to facilitate continued training.
     * When set to false, and done() is called, then the classifier will be cleaned
     * up, resulting in (potentially) a more compact serialized model.
     *
     * @param resume true to enable training to continue at a later point after the
    ...
    /**
     * Returns true if the classifier will be able to be trained further (with more
     * iterations) at a later date. Note that this can potentially make serialized
     * model structures larger, because certain data and data structures might need
     * to be retained to facilitate continued training. When set to false, and
     * done() is called, then the classifier will be cleaned up, resulting in
     * (potentially) a more compact serialized model.
     *
     * @return true to enable training to continue at a later point after the
     *         initial model is built.
     * @throws Exception if finalization cannot be set
     */
    boolean getResume();
