...
public class CostMatrix implements Serializable, RevisionHandler {
    ...
    /**
     * Creates a default cost matrix of a particular size. All diagonal values will
     * be 0 and all non-diagonal values 1.
     * 
        ...
        if (replaceStrings(data)) {
            ...
            if (data.classAttribute().numValues() > 2) {
                ...
                for (int i = 0; i < data.numInstances(); i++) {
                     ...
                     * inst.classAttribute().value((int)inst.classValue()) +" by factor " + factor);
                     */
                }
