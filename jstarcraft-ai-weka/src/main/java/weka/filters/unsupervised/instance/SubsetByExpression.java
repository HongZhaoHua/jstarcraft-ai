 ...
 * - extracting only animals with at least 2 legs from the 'zoo' UCI
 * dataset:<br/>
 * (ATT14 &gt;= 2)<br/>
...
public class SubsetByExpression extends SimpleBatchFilter implements WeightedInstancesHandler, WeightedAttributesHandler {
    ...
    /**
     * SubsetByExpression may return false from input() (thus not making an instance
     * available immediately) even after the first batch has been completed if the
     * user has opted to apply the filter to instances after the first batch (rather
     * than just passing them through).
     * 
     * @return true this filter may remove (consume) input instances after the first
     ...
     * FilteredClassifier, a test instance does not get "consumed" by the filter and
     * a prediction is always generated.
     * 
     * @param b true if the filter should be applied to instances that arrive after
