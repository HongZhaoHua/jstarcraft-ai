 ...
 * the attributes. Useful if one wants to move an attribute to the end of the
 * list of attributes to use it as class attribute (e.g., using "-R
 * 2-last,1").<br/>
 * <br/>
 * It is not only possible to change the order of the attributes. Attributes can
 * also be left out. E.g. if you have 10 attributes, you can generate the
 * following output order: 1,3,5,7,9,10 or 10,1-5.<br/>
 * <br/>
 * You can also duplicate attributes, e.g., for further processing later on:
 * e.g., using 1,1,1,4,4,4,2,2,2 if one needs to process two copies of the
 * attributes with other filters but also needs to keep the original
 * attributes.<br/>
 * <br/>
 * One can simply reverse the order of the attributes via 'last-first'.<br/>
 * <br/>
 * After applying the filter, the index of the class attribute is set to the
