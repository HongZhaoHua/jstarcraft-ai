...
/**
 * <!-- globalinfo-start --> Converts string attributes into a set of numeric
 * attributes representing word occurrence information from the text contained
 * in the strings. The dictionary is determined from the first batch of data
 * filtered (typically training data). Note that this filter is not strictly
 * unsupervised when a class attribute is set because it creates a separate
 * dictionary for each class and then merges them. <br>
 * <br>
 * <!-- globalinfo-end -->
 *
...
public class StringToWordVector extends Filter implements UnsupervisedFilter, OptionHandler, WeightedInstancesHandler {
     ...
     * input format.
     */
    public static final Tag[] TAGS_FILTER = { new Tag(FILTER_NONE, "No normalization"), new Tag(FILTER_NORMALIZE_ALL, "Normalize all data"), new Tag(FILTER_NORMALIZE_TEST_ONLY, "Normalize test data only"), };
    ...
    /**
     * Set the dictionary file to save the dictionary to. A file with an empty path
     * or a path "-- set me --" means do not save the dictionary.
     *
    ...
    /**
     * Set whether to save the dictionary in binary serialized form rather than as
     * plain text
     *
    ...
    /**
     * Gets the rate at which the dictionary is periodically pruned, as a percentage
     * of the dataset size.
     *
    ...
    /**
     * Sets the rate at which the dictionary is periodically pruned, as a percentage
     * of the dataset size.
     *
