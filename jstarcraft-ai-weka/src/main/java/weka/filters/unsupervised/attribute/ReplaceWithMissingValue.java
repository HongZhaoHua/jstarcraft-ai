...
/**
 * <!-- globalinfo-start --> A filter that can be used to introduce missing
 * values in a dataset. The specified probability is used to flip a biased coin
 * to decide whether to replace a particular attribute value in an instance with
 * a missing value (i.e., a probability of 0.9 means 90% of values will be
 * replaced with missing values). This filter only modifies the first batch of
 * data that is processed. The class attribute is skipped by default. <br>
 * <br>
 * <!-- globalinfo-end -->
 *
 ...
 * <pre>
 *  -P &lt;double&gt;
 *  Specify the probability  (default 0.1)
 * </pre>
 * 
 * <pre>
 ...
 */
public class ReplaceWithMissingValue extends SimpleBatchFilter implements UnsupervisedFilter, Randomizable, WeightedAttributesHandler, WeightedInstancesHandler {
    ...
    /**
     * Set the IgnoreClass value. Set this to true if the class index is to be unset
    ...
    /**
     * Gets the IgnoreClass value. If this to true then the class index is to unset
     * before the filter is applied.
     *
    ...
    /**
     * Sets which attributes are to be "nominalized" (only numeric attributes among
     * the selection will be transformed).
     *
