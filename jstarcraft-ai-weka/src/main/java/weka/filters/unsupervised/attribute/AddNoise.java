...
public class AddNoise extends Filter implements UnsupervisedFilter, OptionHandler, Randomizable, WeightedAttributesHandler {
    ...
    public String globalInfo() {
        ...
        return "An instance filter that changes a percentage of a given" + " attribute's values. The attribute must be nominal." + " Missing value can be treated as as a distinct separate value.";
    }
     ...
     * instances are randomly selected using seed. The attribute given by its index
     * is changed from its current value to one of the other possibly ones, also
     * randomly. This is done while leaving the proportion the same. If m_UseMissing
     * is true, missing value is used as a value of its own
     * 
