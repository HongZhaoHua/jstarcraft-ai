...
/**
 * <!-- globalinfo-start --> An instance filter that assumes instances form
 * time-series data and replaces attribute values in the current instance with
 * the difference between the current value and the equivalent attribute
 * attribute value of some previous (or future) instance. For instances where
 * the time-shifted value is unknown either the instance may be dropped, or
 * missing values used. Skips the class attribute if it is set.
 * <p/>
 * <!-- globalinfo-end -->
 * 
...
public class TimeSeriesDelta extends TimeSeriesTranslate {
     ...
     * @throws UnsupportedAttributeTypeException if selected attributes are not
     *                                           numeric.
     */
    ...
    /**
     * Creates a new instance the same as one instance (the "destination") but with
     * some attribute values copied from another instance (the "source")
     *
