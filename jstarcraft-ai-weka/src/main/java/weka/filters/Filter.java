...
public abstract class Filter implements Serializable, CapabilitiesHandler, RevisionHandler, OptionHandler, CapabilitiesIgnorer, CommandlineRunnable {
     ...
     * supervised filters, which "learn" from the first batch and then shouldn't get
     * updated with subsequent calls of batchFinished().
     *
     ...
     * first batch has been completed - such filters should override this method and
     * return true.
     *
    ...
    /**
     * Returns the Capabilities of this filter, customized based on the data. I.e.,
     * if removes all class capabilities, in case there's not class attribute
     * present or removes the NO_CLASS capability, in case that there's a class
     * present.
     *
    ...
    /**
     * Sets the format of output instances. The derived class should use this method
     * once it has determined the outputformat. The output queue is cleared.
     *
     ...
     *
     * @return a copy of the structure (attribute information) of the input format
     *         instances
     */
     ...
     * method for each output instance it makes available. Note that the instance is
     * only copied before it is added to the output queue if it has a reference to a
     * dataset.
     *
     ...
     * Adds an output instance to the queue. The derived class should use this
     * method for each output instance it makes available. Note that the instance is
     * only copied before it is added to the output queue if copyInstance has value
     * true and if the instance has a reference to a dataset.
     *
     * @param instance     the instance to be added to the queue.
     ...
     * attributes of the data will be considered, otherwise only the ones that were
     * provided.
     *
     ...
     * dataset. The Instance must already be assigned to a dataset. This dataset and
     * the destination dataset must have the same structure.
     *
    ...
    /**
     * Takes string/relational values referenced by an Instance and copies them from
     * a source dataset to a destination dataset. The instance references are
     ...
     *                      relational values in the source dataset that will have
     *                      references updated to be valid for the destination
     *                      dataset.
     ...
     * default implementation clears the output format and output queue, and the new
     * batch flag is set. Overriders should call
     ...
     * input() or batchFinished() has returned true. The relation name of the output
     * instances should be changed to reflect the action of the filter (eg: add the
     ...
     * read before producing output, in which case output instances should be
     ...
     *                              format or if there was a problem with the
     *                              filtering.
     */
     ...
     * inputFormat has been re-assigned or new options have been set). This default
     * implementation assumes all instance processing occurs during inputFormat()
     * and input().
     *
     ...
     *                <br/>
     *                -decimal num (the number of decimal places to use in the
     *                output; default = 6) <br/>
     ...
     * If set, filter capabilities are not checked before filter is built (use with
     * caution).
     * <p>
     *
