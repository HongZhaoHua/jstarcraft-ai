 ...
 * @version $Revision: $
 */
@KFStep(name = "Join", category = "Flow", toolTipText = "Performs an inner join on two incoming datasets/instance streams (IMPORTANT: assumes that " + "both datasets are sorted in ascending order of the key fields). If data is not sorted then use" + "a Sorter step to sort both into ascending order of the key fields. Does not handle the case where" + "keys are not unique in one or both inputs.", iconPath = KFGUIConsts.BASE_ICON_PATH + "Join.gif")
public class Join extends BaseStep {
    ...
    /**
     * True if the second input stream is waiting due to a full buffer (incremental
     * mode only)
     */
     ...
     * InstanceHolder
     *
     * @param holder           the InstanceHolder encapsulating the instance and
     *                         it's string values
    ...
    /**
     * Check both buffers and return a joined instance (if possible at this time) or
     * null
     *
     ...
     * @param oneH the first instance holder (in case string attributes are present
     *             and we are running incrementally)
