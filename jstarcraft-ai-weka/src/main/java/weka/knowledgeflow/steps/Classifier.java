 ...
 * @version $Revision: $
 */
@KFStep(name = "Classifier", category = "Classifiers", toolTipText = "Weka classifier wrapper", iconPath = "", resourceIntensive = true)
public class Classifier extends WekaAlgorithmWrapper implements PairedDataHelper.PairedProcessor<weka.classifiers.Classifier> {
    ...
    /**
     * True if we should resent an Updateable classifier at the start of processing
     * for an incoming "instance" stream
     */
        ...
        try {
            ...
            if (!isStopRequested()) {
                ...
                if (getStepManager().numIncomingConnectionsOfType(StepManager.CON_TESTSET) == 0) {
                    ...
                    batchClassifier.setPayloadElement(StepManager.CON_AUX_DATA_PRIMARY_PAYLOAD_NOT_THREAD_SAFE, true);
    ...
    protected void outputGraphData(weka.classifiers.Classifier classifier, int setNum) throws WekaException {
        if (classifier instanceof Drawable) {
            ...
            try {
                ...
                grphTitle = "Set " + setNum + " (" + m_trainedClassifierHeader.relationName() + ") " + grphTitle;
