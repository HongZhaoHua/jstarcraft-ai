...
public abstract class BaseStep implements Step, BaseStepExtender, Serializable {
     ...
     * 
     * @return a default "about" info string if this step uses the KFStep annotation
     *         and null otherwise. Subclasses should override to provide more
     *         comprehensive about info
     */
    ...
    /**
     * Set whether this step is resource intensive (cpu/memory) or not. This affects
     * which executor service is used to execute the step's processing.
     *
    ...
    /**
     * Set whether this step must run single threaded. I.e. on an executor service
     * which has only one worker thread, thus effectively preventing more than one
     * copy of the step executing at any one time.
     *
    ...
    /**
     * Get whether this step must run single threaded. I.e. on an executor service
     * which has only one worker thread, thus effectively preventing more than one
     * copy of the step executing at any one time.
     *
    ...
    /**
     * Start processing. Subclasses should override this method if they can act as a
     * start point in a flow.
     *
     ...
     * {@code isStopRequested()} periodically to see if they should stop processing.
     */
    @Override
    public void stop() {
    ...
    /**
     * When running in a graphical execution environment a step can make one or more
     * popup Viewer components available. These might be used to display results,
     * graphics etc. Returning null indicates that the step has no such additional
     * graphical views. The map returned by this method should be keyed by action
     * name (e.g. "View results"), and values should be fully qualified names of the
     * corresponding StepInteractiveView implementation. Furthermore, the contents
     * of this map can (and should) be dependent on whether a particular viewer
     * should be made available - i.e. if execution hasn't occurred yet, or if a
     * particular incoming connection type is not present, then it might not be
     * possible to view certain results.
     *
     * Viewers can implement StepInteractiveView directly (in which case they need
     * to extends JPanel), or extends the AbstractInteractiveViewer class. The later
     * extends JPanel, uses a BorderLayout, provides a "Close" button and a method
     ...
     * getInteractiveViewers() is the preferred mechanism to specify any interactive
     * viewers, as it does not require Steps to import and instantiate GUI classes.
     * However, in some cases it might be unavoidable (e.g. Groovy script
     * compilation involves custom classloaders), in these cases this method can be
     * used instead.
     *
    ...
    /**
     * Get default settings for the step (if any). Returning null indicates that the
     * step has no user-editable defaults.
     *
