 ...
 * @version $Revision: $
 */
@KFStep(name = "ClassifierPerformanceEvaluator", category = "Evaluation", toolTipText = "Evaluates batch classifiers", iconPath = KFGUIConsts.BASE_ICON_PATH + "ClassifierPerformanceEvaluator.gif")
public class ClassifierPerformanceEvaluator extends BaseStep {
     ...
     * prediction margin
     *
     * @return true if plot data points will be rendered proportional to the size of
    ...
    public synchronized void processIncoming(Data data) throws WekaException {
        try {
            ...
            if (!isStopRequested()) {
                ...
                // submit the task
                EvaluationTask evalTask = new EvaluationTask(this, classifier, trainingData, testData, setNum, m_metricsList, getErrorPlotPointSizeProportionalToMargin(), evalLabel, new EvaluationCallback(), m_costSensitiveEval ? m_matrix : null, getCollectPredictionsForVisAndAUC());
    ...
    protected synchronized void aggregateEvalTask(Evaluation eval, weka.classifiers.Classifier classifier, Instances testData, ClassifierErrorsPlotInstances plotInstances, int setNum, String evalLabel) throws Exception {
        ...
        if (m_setsToGo.get() == m_maxSetNum) {
            ...
            if (getCollectPredictionsForVisAndAUC() && getStepManager().numOutgoingConnectionsOfType(StepManager.CON_VISUALIZABLE_ERROR) > 0) {
            ...
            // threshold data
            if (testData.classAttribute().isNominal() && getCollectPredictionsForVisAndAUC() && getStepManager().numOutgoingConnectionsOfType(StepManager.CON_THRESHOLD_DATA) > 0) {
    ...
    /**
     * Adjust evaluation configuration if an {@code InputMappedClassifier} is being
     * used
     *
     ...
     * @author Mark Hall (mhall{[at]}pentaho{[dot]}com)
     */
    protected static class AggregateableClassifierErrorsPlotInstances extends ClassifierErrorsPlotInstances {
    ...
    protected static class EvaluationTask extends StepTask<Object[]> {
        ...
        public EvaluationTask(Step source, weka.classifiers.Classifier classifier, Instances trainData, Instances testData, int setNum, List<String> metricsList, boolean errPlotPtSizePropToMarg, String evalLabel, EvaluationCallback callback, CostMatrix matrix, boolean collectPreds) {
            ...
            m_classifierDesc = m_classifierDesc.substring(m_classifierDesc.lastIndexOf(".") + 1);
            if (m_classifier instanceof OptionHandler) {
                String optsString = Utils.joinOptions(((OptionHandler) m_classifier).getOptions());
        ...
        public void process() throws Exception {
            ...
            ClassifierErrorsPlotInstances plotInstances = m_collectPreds ? ExplorerDefaults.getClassifierErrorsPlotInstances() : null;
            ...
            if (m_classifier instanceof BatchPredictor && ((BatchPredictor) m_classifier).implementsMoreEfficientBatchPrediction()) {
                ...
                if (m_collectPreds) {
                    ...
                    for (int i = 0; i < m_testData.numInstances(); i++) {
                        eval.evaluationForSingleInstance(predictions[i], m_testData.instance(i), false);
                    }
                }
            } else {
                for (int i = 0; i < m_testData.numInstances(); i++) {
    ...
    /**
     * Callback that gets notified when an evaluation task completes. Passes on the
     * partial evaluation results to be aggregated with the overall results
     */
