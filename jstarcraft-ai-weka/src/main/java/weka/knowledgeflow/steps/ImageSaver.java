 ...
 * @version $Revision: $
 */
@KFStep(name = "ImageSaver", category = "DataSinks", toolTipText = "Save static images to a file", iconPath = KFGUIConsts.BASE_ICON_PATH + "SerializedModelSaver.gif")
public class ImageSaver extends BaseStep {
    ...
    /**
     * Default format to use - read from the settings for this step, and used in the
     * case when the user has selected/left DEFAULT as the format type in the step's
     * options. Must not be set to the type DEFAULT of course :-)
     */
    ...
    public static final class ImageSaverDefaults extends Defaults {
        ...
        public static final Settings.SettingKey DEFAULT_FILE_KEY = new Settings.SettingKey(ID + ".defaultFile", "Default file to save to", "Save to this file if the user has " + "not explicitly set one in the step");
        public static final File DEFAULT_FILE = new File("${user.dir}/image");

        public static final Settings.SettingKey DEFAULT_FORMAT_KEY = new Settings.SettingKey(ID + ".defaultFormat", "Default image format to " + "write", "Default image format to write in the case that the user " + "has explicitly set 'DEFAULT' in the step's options");
