...
public class StepManagerImpl implements StepManager {
    ...
    /**
     * Holds the name of the class of the editor for the managed step. If null/empty
     * then the environment will dynamically generate an editor using the
     * GeneicObjectEditor
     */
    ...
    /**
     * Temporary holder for the x axis visual location of this step. Populated when
     * a flow is loaded if coordinates are present in the step's JSON
    ...
    /**
     * Temporary holder for the y axis visual location of this step. Populated when
     * a flow is loaded if coordinates are present in the step's JSON
     * representation. If running in an interactive graphical environment then a
     * StepVisual will be created and initialized with these values
     */
    ...
    /**
     * Holds the executing environment. Will be able to query to see if environment
     * is headless. Will be able to request a stop of the entire flow.
     */
     ...
     * connections are present, regardless of whether the connection may or may not
     * produce data (e.g. if there is a broken link in the graph further upstream).
     * When this flag is true, the routine adjusts the number of incoming
     * connections of a given type to account for broken upstream links. This is
     * primarily used by the graphical UI in order to change connections from red to
     * grey when rendering.
     */
     ...
     * @throws IllegalStateException if there is no execution environment available
     */
    @Override
    ...
    /**
     * Returns true if this data object marks the end of an incremental stream. Note
     * - does not check that the data object is actually an incremental one of some
     * sort! Just checks to see if the CON_AUX_DATA_INCREMENTAL_STREAM_END flag is
     * set to true or not;
     *
     ...
     * the stream being processed has ended). Final throughput information is
     * printed to the log and status
     *
     * @param data one or more Data events (with appropriate connection type set) to
     *             pass on to downstream connected steps. These are used to carry
     ...
     * Does nothing if this step does not have any connections to the supplied step,
     * or does not have connections to the supplied step of the required type.
     *
    ...
    /**
     * Remove the supplied step from connections (both incoming and outgoing of all
     * types) for the step managed by this manager. Does nothing if the this step
     * does not have any connections to the supplied step
     *
     * @param toDisconnect the step to disconnect
     */
     ...
     * @param step           the source step component associated with the given
     *                       connection type
     */
     ...
     *                       connection type it can't accept the connection at the
     *                       present time
     ...
     * @param step           the target step component that is receiving the given
     *                       connection type
     * @param force          whether to force the connection, even if the target
     *                       step says it can't accept the connection at the present
     *                       time
     * @return true if the connection was successful
     */
     ...
     *
     * @param connectionName the name of the type of connection to remove
     * @param step           the target step component associated with the given
     *                       connection type
     */
     ...
     * specified outgoing connection type. Output listeners are not serialized into
     * the JSON flow when flows are saved.
     *
    ...
    /**
     * Clear all the StepOutputListeners that are registered to receive the supplied
     * connection type.
     * 
     * @param outputConnectionName type of the connection to clear the listeners for
     */
    ...
    public void outputData(String outgoingConnectionName, Data data) throws WekaException {
        if (!isStopRequested()) {
            ...
            boolean copyPrimaryPayload = payloadNotThreadSafe(data) && toNotify.size() > 1;
            boolean first = true;
            if (toNotify != null) {
     ...
     * the connection type in each Data object to be output so that the StepManager
     * knows which connected steps to send the data to. Also notifies any registered
     * {@code StepOutputListeners}. Note that the downstream step(s)'
     * processIncoming() method is called in a separate thread for batch
    ...
    public void outputData(Data... data) throws WekaException {
        if (!isStopRequested()) {
            ...
            for (Data d : data) {
                ...
                if (candidates != null) {
                    boolean copyPrimaryPayload = payloadNotThreadSafe(d) && candidates.size() > 1;
                    boolean first = true;
    ...
    /**
     * Outputs the supplied Data object to the named Step. Does nothing if the named
     * step is not connected immediately downstream of this Step. Sets the supplied
     * connection name on the Data object. Also notifies any StepOutputListeners.
     * 
     ...
     * 
     * @return a list of outgoing connection types that the managed step can produce
     *         (adjusted to take into account any upstream broken connections)
     */
    ...
    /**
     * Attempt to get the incoming structure (as a header-only set of instances) for
     * the named incoming connection type. Assumes that there is only one incoming
     * connection of the named type. If there are zero, or more than one, then null
     * is returned
     *
     * @param connectionName the name of the incoming connection to get the
     *                       structure for
     * @return the structure as a header-only set of instances or null if there are
     *         zero or more than one upstream connected steps producing the named
     *         connection, or if the upstream step can't tell us the structure, or
     *         if the upstream step can't represent the structure of the connection
     *         type as a set of instances.
     *
     * @throws WekaException if a problem occurs
     */
    @Override
    ...
    /**
     * Attempt to retrieve the structure (as a header-only set of instances) for the
     * named incoming connection type. Assumes that there is only one step connected
     * with the supplied incoming connection type.
     *
     ...
     * CON_INFO connections and the managed step is of the supplied class
     *
    ...
    /**
     * Finds a named step in the current flow. Returns null if the named step is not
     * present in the flow
     *
     * @param stepNameToFind the name of the step to find
     * @return the StepManager of the named step, or null if the step does not exist
     *         in the current flow.
     */
    @Override
    ...
    /**
     * Gets a prefix for the step managed by this manager. Used to uniquely identify
     * steps in the status area of the log
     *
    ...
    /**
     * Returns true if the primary payload of the supplied data object is not thread
     * safe.
     *
    ...
    protected static boolean payloadNotThreadSafe(Data data) {
        ...
        return data.getPayloadElement(StepManager.CON_AUX_DATA_PRIMARY_PAYLOAD_NOT_THREAD_SAFE, false);
    }
