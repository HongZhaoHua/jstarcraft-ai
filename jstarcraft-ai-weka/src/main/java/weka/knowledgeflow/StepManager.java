...
public interface StepManager {
    ...
    /**
     * Get the executing environment. This contains information such as whether the
     * flow is running in headless environment, what environment variables are
    ...
    /**
     * Get the number of steps that are connected with the given incoming connection
     * type
     * 
     * @param connectionName the type of the incoming connection
     * @return the number of steps connected with the specified incoming connection
     *         type
     */
    int numIncomingConnectionsOfType(String connectionName);

    /**
     * Get the number of steps that are connected with the given outgoing connection
     * type
     * 
     * @param connectionName the type of the outgoing connection
     * @return the number of steps connected with the specified outgoing connection
     *         type
     */
    int numOutgoingConnectionsOfType(String connectionName);

    /**
     * Get a list of steps that are the source of incoming connections of the given
     * type
     * 
    ...
    /**
     * Step implementations processing batch data should call this to indicate that
     * they have started some processing. Calling this should set the busy flag to
     * true.
     */
    void processing();
    ...
    /**
     * Step implementations processing batch data should call this to indicate that
     * they have finished all processing. Calling this should set the busy flag to
     * false.
     */
    void finished();
     ...
     * have finished processing after a stop has been requested. Calling this should
     ...
     * processing instance streams. Call just after finishing a unit of work for an
     * incoming instance
     */
     ...
     * that are emitting incremental data. Call as the completion of an data stream.
     *
    ...
    /**
     * Log an error message. Always makes it into the log regardless of what logging
     * level the user has specified. Causes all flow execution to halt. Prints an
     * exception to the log if supplied.
     * 
     * @param message the message to log
    ...
    /**
     * Returns true if the step managed by this step manager has been marked as one
     * that must run single-threaded. I.e. in an executor service with one worker
     * thread, thus effectively preventing more than one copy of the step from
     * executing at any one point in time
     *
     * @param mustRunSingleThreaded true if the managed step must run
     *                              single-threaded
     */
