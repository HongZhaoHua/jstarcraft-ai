...
public class BaseExecutionEnvironment implements ExecutionEnvironment {
    ...
    /**
     * The FlowExecutor that will be running the flow. This is not guaranteed to be
     * available from this class until the flow is initialized
     */
    ...
    /**
     * An executor service that steps can use to do work in parallel (if desired) by
     * using {@code StepTask instances}. This executor service is intended for high
     * cpu load tasks, and the number of threads used by it should be kept <= number
    ...
    /**
     * Get the executor that will actually be responsible for running the flow. This
     * is not guaranteed to be available from this execution environment until the
     * flow is actually running (or at least initialized)
     * 
    ...
    /**
     * Set the executor that will actually be responsible for running the flow. This
     * is not guaranteed to be available from this execution environment until the
     * flow is actually running (or at least initialized)
     * 
     ...
     * 
     * @param numThreadsMain     the number of threads to use (level of
     *                           parallelism). <= 0 indicates no limit on
     *                           parallelism
     * @param numThreadsHighLoad the number of threads to use for the high cpu load
     *                           executor service (executes {@code StepTask}
     *                           instances)
     */
    ...
    /**
     * Send the supplied data to the specified step. Base implementation just calls
     * processIncoming() on the step directly. Subclasses may opt to do something
     * different (e.g. wrap data and target step to be executed, and then execute
     * remotely).
     * 
    ...
    public static class BaseExecutionEnvironmentDefaults extends Defaults {
        ...
        public static final Settings.SettingKey STEP_EXECUTOR_SERVICE_NUM_THREADS_KEY = new Settings.SettingKey(KFDefaults.APP_ID + ".stepExecutorNumThreads", "Number of threads to use in the main step executor service", "");
        ...
        public BaseExecutionEnvironmentDefaults() {
            ...
            m_defaults.put(STEP_EXECUTOR_SERVICE_NUM_THREADS_KEY, STEP_EXECUTOR_SERVICE_NUM_THREADS);
            m_defaults.put(RESOURCE_INTENSIVE_EXECUTOR_SERVICE_NUM_THREADS_KEY, RESOURCE_INTENSIVE_EXECUTOR_SERVICE_NUM_THREADS);
        }
    }
}
