...
public abstract class StepTask<T> implements Callable<ExecutionResult<T>>, Serializable {
    ...
    /**
     * The result of executing the task - ready to be populated by subclass's call()
     * implementation
     */
    ...
    /**
     * The callback notifier delegate. Performs the actual notification back to the
     * step
     */
    ...
    /**
     * Set whether this {@code StepTask} is resource intensive (cpu/memory) or not.
     * By default, a {@code StepTask} is resource intensive
     *
    ...
    /**
     * Get whether this {@code StepTask} is resource intensive (cpu/memory) or not.
     * By default, a {@code StepTask} is resource intensive
     *
    ...
    /**
     * Set whether this {@code StepTask} must run single threaded - i.e. only one of
     * these tasks is executing at any one time in the JVM. The Knowledge Flow uses
     * a special executor service with a single worker thread to execute these
     * tasks. This property, if true, overrides isResourceIntensive().
     *
    ...
    /**
     * Get whether this {@code StepTask} must run single threaded - i.e. only one of
     * these tasks is executing at any one time in the JVM. The Knowledge Flow uses
     * a special executor service with a single worker thread to execute these
     * tasks. This property, if true, overrides isResourceIntensive().
     *
    ...
    /**
     * Set the logger to use. This is used by the execution environment - subclasses
     * should call getLogHandler() to do logging
     *
