...
public class LogManager implements LogHandler {
     ...
     * higher will be reported to the log/status. Note that warnings and errors and
     * errors reported via the logWarning() and logError() methods will always be
     * output, regardless of the logging level set
     */
    ...
    public LogManager(Step source) {
        ...
        if (source instanceof WekaAlgorithmWrapper) {
            Object wrappedAlgo = ((WekaAlgorithmWrapper) source).getWrappedAlgorithm();
        ...
        if (source != null) {
            ...
            setLoggingLevel(((StepManagerImpl) source.getStepManager()).getLoggingLevel());
        }
    }
