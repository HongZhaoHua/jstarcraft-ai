...
public class GreedyStepwise extends ASSearch implements RankedOutputSearch, StartSetHandler, OptionHandler {
     ...
     * 
     * @param startSet a string containing a list of attributes (and or ranges), eg.
     *                 1,2,6,10-15.
    ...
    /**
     * Set whether attributes should continue to be added during a forward search as
     * long as merit does not decrease
     * 
     ...
     * This is better than using m_startRanges.getRanges() as the same start set can
     * be specified in different ways from the command line---eg 1,2,3 == 1-3. This
     * is to ensure that stuff that is stored in a database is comparable.
     * 
            ...
            for (i = 0; i < m_numAttribs; i++) {
                ...
                if (z) {
                    ...
                    if (parallel) {
                        ...
                        final SubsetEvaluator theEvaluator = (ASEvaluator instanceof weka.core.ThreadSafe) ? ASEvaluator : (SubsetEvaluator) ASEvaluation.makeCopies(m_ASEval, 1)[0];
     ...
     * during a forward selection search. Individual merit values reflect the merit
     * associated with adding the corresponding attribute to the subset; because of
     * this, merit values may initially increase but then decrease as the best
     * subset is "passed by" on the way to the far side of the search space.
     * 
