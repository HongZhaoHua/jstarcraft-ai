...
public class CfsSubsetEval extends ASEvaluation implements SubsetEvaluator, ThreadSafe, OptionHandler, TechnicalInformationHandler {
    ...
    /**
     * Set whether to pre-compute the full correlation matrix at the outset, rather
     * than computing individual correlations lazily (as needed) during the search.
     * 
    ...
    /**
     * Get whether to pre-compute the full correlation matrix at the outset, rather
     * than computing individual correlations lazily (as needed) during the search.
     * 
    ...
    private double num_nom2(int att1, int att2) {
        ...
        for (i = 0, temp = 0.0; i < nx; i++) {
            ...
            /*
             * if there is zero variance for the numeric att at a specific level of the
             * catergorical att then if neither is the class then make this correlation at
             * this level maximally bad i.e. 1.0. If either is the class then maximally bad
             * correlation is 0.0
             */
            else {
                if (att1 != m_classIndex && att2 != m_classIndex) {
    ...
    /**
     * Calls locallyPredictive in order to include locally predictive attributes (if
     * requested).
     * 
