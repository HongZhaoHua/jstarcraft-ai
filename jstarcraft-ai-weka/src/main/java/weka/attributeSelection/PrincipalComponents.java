...
public class PrincipalComponents extends UnsupervisedAttributeEvaluator implements AttributeTransformer, OptionHandler {
    ...
    /**
     * the amount of variance to cover in the original data when retaining the best
     * n PC's
     */
    ...
    /**
     * Set whether to center (rather than standardize) the data. If set to true then
    ...
    /**
     * Get whether to center (rather than standardize) the data. If true then PCA is
     * computed from the covariance rather than correlation matrix.
     * 
    ...
    /**
     * Gets the proportion of total variance to account for when retaining principal
     * components
     * 
    ...
    private void buildAttributeConstructor(Instances data) throws Exception {
        ...
        /*
         * for (int i = 0; i < m_numAttribs; i++) { for (int j = 0; j < m_numAttribs;
         * j++) { System.err.println(v[i][j] + " "); } System.err.println(d[i]); }
         */
     ...
     * instances. This is so that AttributeSelection can determine the structure of
     * the transformed data without actually having to get all the transformed data
     * through transformedData().
     *
     * @return the header of the transformed data.
     * @throws Exception if the header of the transformed data can't be determined.
     */
    @Override
    ...
    /**
     * Evaluates the merit of a transformed attribute. This is defined to be 1 minus
     * the cumulative variance explained. Merit can't be meaningfully evaluated if
     * the data is to be transformed back to the original space.
     *
