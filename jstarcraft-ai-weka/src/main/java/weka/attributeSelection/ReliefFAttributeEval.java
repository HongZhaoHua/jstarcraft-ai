...
public class ReliefFAttributeEval extends ASEvaluation implements AttributeEvaluator, OptionHandler, TechnicalInformationHandler {
    ...
    /**
     * Used to hold the probability of a different class val given nearest instances
     * (numeric class)
     */
     ...
     * instance in question. Each entry holds exp(-((rank(r_i, i_j)/sigma)^2)) where
     * rank(r_i,i_j) is the rank of instance i_j in a sequence of instances ordered
     * by the distance from r_i. sigma is a user defined parameter, default=20
     **/
    ...
    /**
     * Find the K nearest instances to supplied instance if the class is numeric, or
     * the K nearest Hits (same class) and Misses (K from each of the other classes)
     * if the class is discrete.
     * 
    ...
    private void findKHitMiss(int instNum) {
        ...
        for (i = 0; i < m_numInstances; i++) {
            if (i != instNum) {
                ...
                /*
                 * if we already have stored knn for this class then check to see if this
                 * instance is better than the worst
                 */
                {
