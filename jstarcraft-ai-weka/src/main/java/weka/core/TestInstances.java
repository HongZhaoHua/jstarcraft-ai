...
public class TestInstances implements Cloneable, Serializable, OptionHandler, RevisionHandler {
    ...
    /**
     * whether to have no class, e.g., for clusterers; otherwise the class attribute
    ...
    /**
     * returns the current set CapabilitiesHandler to generate the dataset for, can
     * be null
     * 
        ...
        if (getMultiInstance()) {
            ...
            for (int i = 0; i < getNumInstances(); i++) {
                ...
                if ((clsIndex != NO_CLASS) && (m_Data.classAttribute().isString()))
                    classVal++; // Hack to make regression tests pass after eliminating dummy value for string
                                // attributes
