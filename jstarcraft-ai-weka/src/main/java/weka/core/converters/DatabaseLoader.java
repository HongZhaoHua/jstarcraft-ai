...
public class DatabaseLoader extends AbstractLoader implements BatchConverter, IncrementalConverter, DatabaseConverter, OptionHandler, EnvironmentHandler {
    ...
    /**
     * The user defined query to load instances. (form: SELECT *|&ltcolumn-list&gt;
     * FROM &lttable&gt; [WHERE &lt;condition&gt;])
     */
    protected String m_query = "Select * from Results0";

    /**
     * Flag indicating that pseudo incremental mode is used (all instances load at
     * once into main memeory and then incrementally from main memory instead of the
     * database)
     */
     ...
     * loading depending on jdbc metadata information. Set flag to false if no check
     * is required or if jdbc metadata is not complete.
     */
    ...
    /**
     * Decides which SQL statement to limit the number of rows should be used. DBMS
     * dependent. Algorithm just tries several possibilities.
     */
     ...
     * Ensures a unique ordering of instances for indremental loading.
     */
    ...
    /**
     * Name of the primary key column that will allow unique ordering necessary for
     * incremental loading. The name is specified in the DatabaseUtils file.
     */
     ...
     * @return the query
     */
    @OptionMetadata(displayName = "Query", description = "The query to execute", displayOrder = 4)
     ...
     *
     * @param value the custom props file to load database parameters from, use null
     *              or directory to disable custom properties.
     */
     ...
     */
    @OptionMetadata(displayName = "Create sparse instances", description = "Return sparse " + "rather than normal instances", displayOrder = 6)
     ...
     * detected. The key is needed to order the instances uniquely for an inremental
     * loading. If an existing key cannot be detected, use -P option.
     * 
                ...
                for (int i = 1; i <= numAttributes; i++) {
                    ...
                    case DatabaseConnection.STRING:
                        ...
                        if (m_DataBaseConnection.execute(query) == true) {
                            ...
                            // System.err.println("Count for nominal values cannot be calculated. Attribute
                            // "+columnName+" treated as String.");
                            attributeTypes[i - 1] = Attribute.STRING;
                            break;
                        }
     ...
     * 
     * @param structure the dataset header information, will get updated in case of
