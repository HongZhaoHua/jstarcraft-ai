...
public class WekaPackageClassLoaderManager {
    ...
    /**
     * Lookup for classloaders keyed by class names from top-level package jar files
     */
    ...
    /**
     * Injects the MTJ core classes into the root classloader. This is so that they
     * are visible to the MTJ native library loader (if a MTJ native package is
     * installed).
     */
    ...
    /**
     * Return an instantiated instance of the supplied class name. This method will
     * attempt to find a package that owns the named class first, before falling
     * back on the current and then parent class loader. Use this method instead of
    ...
    /**
     * Return the class object for the supplied class name. This method will attempt
     * to find a package that owns the named class first, before falling back on the
     * current, and then parent, class loader. Use this method instead of
     * Class.forName().
     * 
     ...
     * the core weka classes) as an array of URLs. This is primarily used by Weka's
     * dynamic class discovery mechanism, so that all Weka schemes on the classpath
     * can be discovered. If the Weka class loader is an instance of URLClassLoader
     * then the URLs encapsulated within are returned. Otherwise, if the system
     * class loader is the classloader that loads Weka then the entries from
     * java.class.path are returned. Failing that, we assume that Weka and any other
     * supporting classes (not including packages) can be found in the
     * WEKA_CLASSPATH environment variable. This latter case can be used to handle
     * the situation where the weka.jar is loaded by a custom (non-URLClassLoader)
     * withn an app server (for example).
     * 
     * @return an array of URLs containing the entries available to the classloader
     *         that loads the core weka classes
     */
     ...
     * similar). In this case, if the child classloader is a URLClassLoader then the
     * original class discovery mechanism will work; if not, then the WEKA_CLASSPATH
     * environment variable will need to be set to point to the location of the
     * weka.jar file on the file system.
     *
    ...
    /**
     * Splits a supplied string classpath and returns an array of URLs representing
     * the entries.
     *
     ...
     * should not be looking directly for such third-party classes. Instead, if they
     * require third-party classes they should reference them directly (and compile
     * against the libraries in question), and then either include the third party
     * libraries in their own lib directory or declare a dependency on the package
     * that contains them.
     * 
    ...
    /**
     * Attempts to find the named class. Tries the Weka classloader first (for core
     * Weka classes and general Java classes) and then tries package classloaders
     * (with respect to the globally visible classes contained in their top-level
     * jar files). Note that this method will not find classes contained in
     * third-party libraries that reside in lib directories).
     * 
     ...
     * classloaders. Note that it will only find resources contained in a package's
     * top-level jar file(s).
     * 
     ...
     * Note that this considers the Weka classloader and package classloaders (with
     * respect to resources contained in their top-level jar file(s))
     * 
     ...
     *
     * @param name the name of the resource to find
     * @return an enumeration of URLs to the resource, or null if the resource could
     *         not be located
     */
    ...
    /**
     * Try to find a resource from the classloader for the named package. Note that
     * this will traverse transitive package dependencies
     * 
    ...
    /**
     * Inject all classes from the supplied input stream into the root classloader.
     * Assumes that the zip entries can be read from the input stream (i.e. a
     * jar/zip file is the target)
     * 
    ...
    /**
     * Inject classes from the supplied jar file into the Weka or root classloader.
     * 
     * @param jarPath                 the path to the jar file to process
     * @param classJarPaths           an optional list of paths to classes in the
     *                                jar file to inject (if null then all classes
     *                                in the jar file are injected)
     * @param classes                 an optional list of fully qualified class
     *                                names. This should correspond to the paths in
     *                                classJarPaths, but contain just the class name
     *                                along with slashes replaced by "." and the
     *                                .class extension removed.
     * @param injectToRootClassLoader true if the classes are to be injected into
     *                                the root classloader rather than the Weka
     *                                classloader
     * @throws Exception if a problem occurs
     */
     ...
     *
     * @param jarStream               input stream to process. Is expected that
     *                                jar/zip entries can be read from the stream
     * @param classJarPaths           an optional list of paths to classes in the
     *                                jar file to inject (if null then all classes
     *                                in the jar file are injected)
     * @param classes                 an optional list of fully qualified class
     ...
     * (i.e. a classloader for each dependency is present). This method is called by
     * WekaPackageManager after loading all packages, but before dynamic class
     * discovery is invoked. This takes care of any issues that might arise from the
     * user toggling the load status of a package, or perhaps manually installing
     * packages that preclude one another, which might make one or more dependencies
     * unavailable
     */
