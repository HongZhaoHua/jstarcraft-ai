...
public class PMMLFactory {
    ...
    protected enum ModelType {
        UNKNOWN_MODEL("unknown"), REGRESSION_MODEL("Regression"), GENERAL_REGRESSION_MODEL("GeneralRegression"), NEURAL_NETWORK_MODEL("NeuralNetwork"), TREE_MODEL("TreeModel"), RULESET_MODEL("RuleSetModel"), SVM_MODEL("SupportVectorMachineModel");
     ...
     * @return the transformation dictionary or null if there is none defined in the
     *         Document
     * @throws Exception if there is a problem getting the transformation dictionary
     */
     ...
     * @param model          the Element encapsulating the model part of the
     *                       Document
     * @param dataDictionary the data dictionary as an Instances object
    ...
    public static void main(String[] args) {
        try {
            ...
            /*
             * PMMLModel model = getPMMLModel(args[0], null); System.out.println(model); if
             * (args.length == 2) { // load an arff file Instances testData = new
             * Instances(new java.io.BufferedReader(new java.io.FileReader(args[1])));
             * Instances miningSchemaI = model.getMiningSchema().getFieldsAsInstances(); if
             * (miningSchemaI.classIndex() >= 0) { String className =
             * miningSchemaI.classAttribute().name(); for (int i = 0; i <
             * testData.numAttributes(); i++) { if
             * (testData.attribute(i).name().equals(className)) { testData.setClassIndex(i);
             * System.out.println("Found class " + className + " in test data."); break; } }
