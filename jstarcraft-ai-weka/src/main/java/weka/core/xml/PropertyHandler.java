...
public class PropertyHandler implements RevisionHandler {
    ...
    /**
     * lists for a class the properties allowed to use for setting and getting. if a
     * class is not listed, then all get/set-methods are allowed.<br>
    ...
    /**
     * returns an enumeration of the stored display names and classes of properties
     * to ignore.<br>
     * <b>NOTE:</b> String and Class Objects are mixed in this enumeration,
     * depending whether it is a global property to ignore or just one for a certain
     * class!
     * 
    ...
    /**
     * adds the given display name of a property to the ignore list. Can either be a
     * complete path (e.g. <code>__root__.options</code>) or only a property name
     * (e.g. <code>options</code>). In the latter case it matches all occurences of
     * this display name.
     * 
     ...
     * whether the removing was succesful, i.e. whether the display name was in the
     * list.
     * 
     ...
     * property. It only checks for this certain class and no derivative classes. If
    ...
    /**
     * adds the given property (display name) to the list of allowed properties for
     * the specified class.
     * 
