...
public class CheckClusterer extends CheckScheme {
    ...
    /**
     * Checks whether the scheme correctly initialises models when buildClusterer is
     * called. This test calls buildClusterer with one training dataset.
     * buildClusterer is then called on a training set with different structure, and
     * then again with the original training set. If the equals method of the
    ...
    /**
     * Checks whether the clusterer can handle instance weights. This test compares
     * the clusterer performance on two datasets that are identical except for the
     * training weights. If the results change, then the clusterer must be using the
     * weights. It may be possible to get a false positive from this test if the
     * weight changes aren't significant enough to induce a change in clusterer
     ...
     * @param relationalPredictor if true use relational predictor attributes
     * @param multiInstance       whether multi-instance is needed
     * @param predictorMissing    true if we know the clusterer can handle (at
