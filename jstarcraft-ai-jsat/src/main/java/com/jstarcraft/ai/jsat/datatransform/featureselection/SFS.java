...
public class SFS implements DataTransform {
     ...
     *                    feature subset
     * @param maxIncrease the maximum tolerable increase in error when a feature is
     *                    added
     */
    ...
    private void search(int minFeatures, int maxFeatures, DataSet dataSet, int folds) {
        ...
        while (catSelected.size() + numSelected.size() < maxFeatures) {
            if (SFSSelectFeature(available, dataSet, catToRemove, numToRemove, catSelected, numSelected, learner, folds, rand, bestScore, minFeatures) < 0)
                break;
    ...
    static protected int SFSSelectFeature(Set<Integer> available, DataSet dataSet, Set<Integer> catToRemove, Set<Integer> numToRemove, Set<Integer> catSelecteed, Set<Integer> numSelected, Object evaluater, int folds, Random rand, double[] PbestScore, int minFeatures) {
        ...
        if (curBestScore <= 1e-14 && PbestScore[0] <= 1e-14 && catSelecteed.size() + numSelected.size() >= minFeatures)
            return -1;
        if (curBestScore < PbestScore[0] || catSelecteed.size() + numSelected.size() < minFeatures || Math.abs(PbestScore[0] - curBestScore) < 1e-3) {
     ...
     * feature is added
     *
     * @param maxIncrease the maximum allowable the maximum tolerable increase in
     *                    error when a feature is added
     */
    ...
    /**
     *
     * @return the maximum allowable the maximum tolerable increase in error when a
     *         feature is added
     */
