...
public interface ClusterEvaluation {
     ...
     * 
     * @param designations the array that stores the cluster assignments for each
     ...
     *
     * @param dataSets a list of lists, where the size of the first index indicates
     *                 the the number of clusters, and the list at each index is the
    ...
    /**
     * The {@link #evaluate(java.util.List) evaluate} methods mandate a score to be
     * returned in such a way that lower values are better. This method takes the
     * value returned by an evaluate method, and returns the score as naturally
     * defined by the cluster evaluation method. This is useful for when some
     * algorithms return scores where higher is better, and we wish to display the
     * scores in their intended form.
     *
