...
import static com.jstarcraft.ai.jsat.classifiers.trees.TreePruner.prune;
...
import com.jstarcraft.ai.jsat.classifiers.trees.TreePruner.PruningMethod;
import com.jstarcraft.ai.jsat.exceptions.FailedToFitException;
import com.jstarcraft.ai.jsat.exceptions.ModelMismatchException;
...
public class DecisionTree implements Classifier, Regressor, Parameterized, TreeLearner {
    ...
    /**
     * Base decision stump used to clone so that we can keep certain features inside
     * the stump instead of duplicating them here.
     */
    ...
    public double regress(DataPoint data) {
        if (data.numNumericalValues() != root.stump.numNumeric() || data.numCategoricalValues() != root.stump.numCategorical())
            throw new ModelMismatchException("Tree expected " + root.stump.numNumeric() + " numeric and " + root.stump.numCategorical() + " categorical features, instead received data with " + data.numNumericalValues() + " and " + data.numCategoricalValues() + " features respectively");
    ...
    /**
     * Creates a Decision Tree that uses {@link PruningMethod#REDUCED_ERROR} pruning
     * on a held out 10% of the data.
     */
    public DecisionTree() {
     ...
     * @param minSamples     the minimum number of samples needed to continue
     *                       branching
     * @param pruningMethod  the method of pruning to use after construction
     * @param testProportion the proportion of the data set to put aside to use for
     *                       pruning
     */
     ...
     * classification data. The exact behavior not identical, and certain base cases
     * may not behave in the exact same manner. However, it uses all of the same
     ...
     * 
     * @param pruningMethod the method of pruning that will be used after tree
     *                      construction
     * @see PruningMethod
     */
    ...
    /**
     * Sets the proportion of the training set that is put aside to perform pruning
     * with. <br>
    ...
    protected void trainC(ClassificationDataSet dataSet, Set<Integer> options, boolean parallel) {
        if (dataSet.size() < minSamples)
            throw new FailedToFitException("There are only " + dataSet.size() + " data points in the sample set, at least " + minSamples + " are needed to make a tree");
     ...
     */
    protected Node makeNodeC(ClassificationDataSet dataPoints, final Set<Integer> options, final int depth, final boolean parallel, final ModifiableCountDownLatch mcdl) {
     ...
     * 
     * @param dataPoints the list of data points paired with their associated real
     *                   value
     * @param options    the attributes that this tree may select from
     * @param depth      the current depth of the tree
