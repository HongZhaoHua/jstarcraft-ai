...
import com.jstarcraft.ai.jsat.classifiers.boosting.Bagging;
...
public class RandomForest implements Classifier, Regressor, Parameterized {
     ...
     * heuristic of sqrt(N) or N/3 should be used for classification and regression
     * respectively. This value should be set away from -1 before training work
     ...
     * 
     * @param featureSamples the number of features to randomly select for each tree
     *                       in the forest.
    ...
    /**
     * Returns the number of rounds of boosting that will be done, which is also the
     * number of base learners that will be trained
     * 
     * @return the number of rounds of boosting that will be done, which is also the
     *         number of base learners that will be trained
     */
     ...
     * 
     * @param useOutOfBagError <tt>true</tt> to compute the out of bag error,
    ...
    /**
     * Random Forest can obtain an unbiased estimate of feature importance using a
     * {@link TreeFeatureImportanceInference} method on the out-of-bag samples
     * during training. Since each tree will produce a different importance score,
     * we also get a set of statistics for each feature rather than just a single
     * score value. These are only computed if
     * {@link #setUseOutOfBagImportance(boolean) } is set to <tt>true</tt>.
     * 
     * @return an array of size equal to the number of features, each
     *         {@link OnLineStatistics} describing the statistics for the importance
     *         of each feature. Numeric features start from index 0, and categorical
     *         features start from the index equal to the number of numeric
     *         features.
     */
     ...
     * during training.
     *
     * @param useOutOfBagImportance <tt>true</tt> to compute the out of bag feature
     *                              importance, <tt>false</tt> to skip it
     */
     ...
     * after training. Otherwise, it will return the out of bag error estimate after
     * training has completed. For classification problems, this is the 0/1 loss
     * error rate. Regression problems return the mean squared error.
     * 
    ...
    /**
     * Does the actual set up and training. {@link #predicting } and {@link #forest}
     * should be set up appropriately first. Everything else is handled by this and
     * {@link LearningWorker}
     * 
