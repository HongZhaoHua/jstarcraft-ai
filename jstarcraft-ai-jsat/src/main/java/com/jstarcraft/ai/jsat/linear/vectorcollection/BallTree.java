...
import com.jstarcraft.ai.jsat.clustering.TRIKMEDS;
...
public class BallTree<V extends Vec> implements IncrementalCollection<V>, DualTree<V> {
    ...
    public enum ConstructionMethod {
        /**
         * This represents a top-down construction approach, that can be used for any
         * distance metric. At each branch the children are given initial prototypes.
         * The left child has a prototype selected as the point farthest from the pivot,
         * and the right child the point farthest from the left's. The points are split
         * based on which prototype thye are closest too. The process continues
         * recursively. <br>
         *
         * See: Moore, A. W. (2000). The Anchors Hierarchy: Using the Triangle
         * Inequality to Survive High Dimensional Data. In Proceedings of the Sixteenth
         * Conference on Uncertainty in Artificial Intelligence (pp. 397–405). San
         * Francisco, CA, USA: Morgan Kaufmann Publishers Inc. Retrieved from
         * http://dl.acm.org/citation.cfm?id=2073946.2073993 for details.
         */
        TOP_DOWN_FARTHEST,
        /**
         * This represents a top-down construction approach similar to a KD-tree's
         * construction. It requires a metric where it has access to meaningful feature
         * values. At each node, the dimension with the largest spread in values is
         * selected. Then the split is made based on sorting the found feature into two
         * even halves.<br>
         * See: Omohundro, S. M. (1989). Five Balltree Construction Algorithms (No.
         * TR-89-063).
         */
        KD_STYLE,
        /**
         * This represents a "middle-out" construction approach. Computational is works
         * best when the {@link PivotSelection#CENTROID centroid} pivot selection method
         * can be used.<br>
         * See: Moore, A. W. (2000). The Anchors Hierarchy: Using the Triangle
         * Inequality to Survive High Dimensional Data. In Proceedings of the Sixteenth
         * Conference on Uncertainty in Artificial Intelligence (pp. 397–405). San
    ...
    public enum PivotSelection {
        /**
         * This method selects the pivot by taking the centroid (average) of all the
         * data within a node. This method may not be applicable for all metrics, and
         * can't be used for once for which there is no computable average.
         */
        CENTROID {
            @Override
        ...
        /**
         * This method selects the pivot by searching for the medoid of the data. This
         * can be used in all circumstances, but may be slower.
         */
        MEDOID {
            @Override
            public Vec getPivot(boolean parallel, List<Integer> points, List<? extends Vec> data, DistanceMetric dm, List<Double> cache) {
        ...
        /**
         * This method selects the pivot by searching for an approximate medoid of the
    ...
    private Node build_far_top_down(List<Integer> points, boolean parallel) {
        ...
        int f1 = ParallelUtils.streamP(points.stream(), parallel).map(i -> new IndexDistPair(i, dm.dist(i, branch.pivot, branch.pivot_qi, allVecs, cache))).max(IndexDistPair::compareTo).orElse(new IndexDistPair(0, 0.0)).indx;
        ...
        if (left_children.isEmpty() || right_children.isEmpty()) {
            // This can happen if all the points have the exact same value, so all distances
            // are zero.
    ...
    private Node build_kd(List<Integer> points, boolean parallel) {
        ...
        IndexDistPair maxSpread = ParallelUtils.range(D, parallel).mapToObj(d -> {
    ...
    private Node build_anchors(List<Integer> points, boolean parallel) {
        // Ceiling to avoid issues with points rounding down to k=1, causing an infinite
        // recusion
        ...
        for (int k = 1; k < K; k++) {
            /*
             * How is the new anchor a^new chosen? We simply find the current anchor
             * a^maxrad with the largest radius, and choose the pivot of a^new to be the
            ...
            for (int j = 0; j < k; j++) {
                ...
                while (ownedIter.hasPrevious()) {
                    ...
                    if (dist_ak_x < dist_aj_x)// we can steal this point!
                    {
                        ...
                        // "we can deduce that the remainder of the points in ai's list cannot possibly
                        // be stolen"
                        break;
                    }
                }
        ...
        // Now we have sqrt(R) anchors. Lets do the middle-down first, creating Nodes
        // for each anchor
    ...
    public void insert(V x) {
        ...
        while (curNode != null) {
            ...
            if (curNode instanceof com.jstarcraft.ai.jsat.linear.vectorcollection.BallTree.Leaf) {
    ...
    private abstract class Node implements Cloneable, Serializable, Iterable<Integer>, IndexNode<Node> {
        ...
        public double[] minMaxDistance(Node other) {
            ...
            return new double[] { d - this.radius - other.radius, d + this.radius + other.radius };
        }
...
//        }
    ...
    private Node cloneChangeContext(Node toClone) {
        if (toClone != null)
            if (toClone instanceof com.jstarcraft.ai.jsat.linear.vectorcollection.BallTree.Leaf)
