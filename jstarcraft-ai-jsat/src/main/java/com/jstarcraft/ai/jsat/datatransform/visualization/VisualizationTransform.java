...
public interface VisualizationTransform extends Cloneable, Serializable {
    ...
    /**
     * Sets the target dimension to embed new dataset to. Many visualization methods
     * may only support a target of 2 or 3 dimensions, or only one of those options.
     * For that reason a boolean value will be returned indicating if the target
     * size was acceptable. If not, no change to the object will occur.
     *
     * @param target the new target dimension size when
     *               {@link #transform(com.jstarcraft.ai.jsat.DataSet) } is called.
