...
//                    B_nc = computeKnnBound(query.getChild(c), numNeighbors, allPriorities, query_B_cache);
        ...
        // then add the remaining 2 terms, which are constant for a given Node Q. If no
        // valid points, bound remains infinite
...
//            System.out.println(bound_3);
    ...
    default public void search(VectorCollection<V> VC, double r_min, double r_max, List<IntList> neighbors, List<DoubleList> distances, boolean parallel) {
        ...
        ScoreDT score = (IndexNode ref, IndexNode query) -> {
            ...
            if (d_min > r_max || d_max < r_min)// If min dist is greater than max-range, or max distance is greater than
                                               // min-range, we can prune
                return Double.NaN;
        ...
        // Range search dosn't benefit from improved search order. So use basic one and
     ...
     * contain points in branching nodes. The dual tree traversal assumes all points
     * belong in leaf nodes. This fixes that by wraping an IndexNode to behave as if
     * all points owned within a branch really belong to a special extra "self"
     * child.
     *
     * @param <N>
     */
    ...
    class DualTreeTraversalAction extends RecursiveAction implements Comparable<DualTreeTraversalAction> {
        ...
        protected void compute() {
            /*
             * B/c of fork-join framework, we can't do the ScoreDTLazy check before placing
             * them into the execution que. So we will do them at the root no upon
             * ourselves. We can do that b/c priority is the score for the pair of
             * IndexNodes we are about to process!
             */
