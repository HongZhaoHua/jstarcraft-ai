...
import static com.jstarcraft.ai.jsat.clustering.ClustererBase.createClusterListFromAssignmentArray;
...
import com.jstarcraft.ai.jsat.clustering.dissimilarity.LanceWilliamsDissimilarity;
import com.jstarcraft.ai.jsat.clustering.dissimilarity.WardsDissimilarity;
...
public class NNChainHAC implements KClusterer {
     ...
     * metric. The correctness guarantees may not hold for distances other than the
     * {@link EuclideanDistance Euclidean} distance, which is the norm for
    ...
    private double getDist(int a, int j, int[] size, List<Vec> vecs, DoubleList cache, List<Int2DoubleMap> d_xk) {
        if (size[j] == 1 && size[a] == 1)
            ...
            // a is the one we are using over and over, its more likely to have the valu -
            // check it first
            if (d_xk.get(a) != null) {
...
//            System.out.println("Removed " + a + " " + b + " S=" + S + " chain=" + IntList.view(chain, chainPos));
            ...
            for (int i = Math.max(0, chainPos - 5); i < chainPos; i++)// bug in paper? we end with [a, b, a] in the chain, but one of them is a bad
                                                                      // index now
                if (chain[i] == removed)
                    chain[i] = n;
            // 23: size[n]←size[a]+size[b]
            final int size_a = size[a], size_b = size[b];
            // set defered till later to make sure we don't muck anything needed in
            // computatoin
    ...
    /**
     * After clustering, we need to fix up the merge order - since the NNchain only
     * gets the merges correct, not their ordering. This also figures out what
    ...
    private void fixMergeOrderAndAssign(double[] mergedDistance, IntList merge_kept, IntList merge_removed, int lowK, final int N, int highK, int[] designations) {
        // Now that we are done clustering, we need to re-order the merges so that the
        // smallest distances are mergered first
        ...
        /*
         * Keep track of the average dist when merging, mark when it becomes abnormaly
         * large as a guess at K
         */
