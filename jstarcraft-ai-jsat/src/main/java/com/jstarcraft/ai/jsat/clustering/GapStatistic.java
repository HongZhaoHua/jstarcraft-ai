...
import com.jstarcraft.ai.jsat.clustering.evaluation.IntraClusterSumEvaluation;
import com.jstarcraft.ai.jsat.clustering.evaluation.intra.SumOfSqrdPairwiseDistances;
import com.jstarcraft.ai.jsat.clustering.kmeans.HamerlyKMeans;
import com.jstarcraft.ai.jsat.clustering.kmeans.KMeans;
...
public class GapStatistic extends KClustererBase implements Parameterized {
     ...
     * 
     * @param base the base clustering method to use for any individual number of
     *             clusters
     */
     ...
     *                   of clusters
     * @param PCSampling {@code true} if the Gap statistic should be computed from a
     *                   PCA transformed space, or {@code false} to go with the
     *                   uniform bounding hyper cube.
     */
    ...
    /**
     * Returns the array of empirical <i>log(W<sub>k</sub>)</i> scores computed from
     * the data set last clustered. <br>
    ...
    public int[] cluster(DataSet dataSet, int lowK, int highK, boolean parallel, int[] designations) {
        ...
        // TODO we dont need all values in [1, lowK-1) in order to get the gap statistic
        // for [lowK, highK]. So lets not do that extra work.
        ...
        // Step 2:
        // use online statistics and run through all K for each B, so that we minimize
        // the memory use
        ...
        int k_first = -1;
        int biggestGap = 0;// used as a fall back incase the original condition can't be satisfied in the
                           // specified range
