...
import static com.jstarcraft.ai.jsat.utils.ClosedHashingUtil.DELETED;
import static com.jstarcraft.ai.jsat.utils.ClosedHashingUtil.EMPTY;
import static com.jstarcraft.ai.jsat.utils.ClosedHashingUtil.EXTRA_INDEX_INFO;
...
public class IntSet extends AbstractSet<Integer> implements Serializable {
    ...
    private int used = 0;
    /**
     * Number of indices marked "free". It is possible for used= 0 and free = 0, if
     * everything was deleted. When nothig is free, we need to re-insert and clear
     * the index to avoid corner case infinite loop that was not accounted for in
     * original algo description
     */
    private int free = 0;
    private byte[] status;
    private int[] keys;
    ...
    /**
     * Gets the index of the given key. Based on that {@link #status} variable, the
     * index is either the location to insert OR the location of the key.
     * 
     ...
     * @param inserting {@code true} if a value is being inserted into the index,
     *                  false otherwise
     * @return the mixed long containing the index of the first DELETED position and
     *         the position that the key is in or the first EMPTY position found
     */
    ...
    private void enlargeIfNeeded() {
        ...
        if (used > keys.length * loadFactor)
            newSize = getNextPow2TwinPrime(status.length * 3 / 2);// it will actually end up doubling in size since we have twin primes spaced
                                                                  // that was
    ...
    public Iterator<Integer> iterator() {
        ...
        return new Iterator<Integer>() {
            ...
            @Override
            public void remove() {
                // its ok to just call remove b/c nothing is re-ordered when we remove an
                // element, we just set the status to DELETED
