package com.jstarcraft.ai.jsat.text.topicmodel;
 ...
 * "http://videolectures.net/site/normal_dl/tag=83534/nips2010_1291.pdf">
 * here</a></li>
 ...
 * <a href=
 * "https://groups.yahoo.com/neo/groups/vowpal_wabbit/conversations/topics/250">here</a></li>
 * </ul>
 * 
 * @author Edward Raff
 */
public class OnlineLDAsvi implements Parameterized {
     ...
     * {@link #setVocabSize(int) vocabulary size} must be set before it can be used.
     */
    public OnlineLDAsvi() {
        K = D = W = -1;
    }
    ...
    /*
     * Using lists instead of matricies b/c we want to use HDP-OnlineLDAsvi for when
     * k is not specified <br> One row for each K topics, each vec is |W| long
     * <br><br> Lambda is also used to determine if the rest of the structures need
     * to be re-intialized. When lambda is {@code null} the structures need to be
     * reinitialized.
     */
    ...
    /**
     * Returns the number of topics to learn, or {@code -1} if <i>this</i> object is
     * not ready to learn
     * 
    ...
    /**
     * A learning rate constant to control the influence of early iterations on the
     * solution. Larger values reduce the influence of earlier iterations, smaller
     * values increase the weight of earlier iterations.
     * 
     * @param tau0 a learning rate parameter that must be greater than 0 (usually at
     *             least 1)
     */
    ...
    /**
     * Returns the topic vector for a given topic. The vector should not be altered,
     * and is scaled so that the sum of all term weights sums to one.
     * 
    ...
    /**
     * From the 2013 paper, see expectations in figure 5 on page 1323, and equation
     * (27) on page 1325 See also equation 6 in the 2010 paper. 2013 paper figure 5
     * seems to be a typo
     * 
    ...
    public void update(final List<Vec> docs, ExecutorService ex) {
         ...
         * so we can interleave the "M" step into the final update accumulation to avoid
         * temp space allocation and more easily exploit sparsity
         * 
         */
        ...
        /*
         * As described in the 2010 paper, this part is the "E" step if we view it as an
         * EM algorithm
         */
        ...
        /*
         * See note on page 3 from 2010 paper: "In practice, this algorithm converges to
         * a better solution if we reinitialize γ and φ before each E step"
         */
    ...
    /**
     * Updates the Beta vectors associated with the {@link #gammaLocal gamma} topic
     * distributions so that they can be used to update against the given batch of
     * documents. Once updated, the Betas are the only items needed to perform
     * updates from the given batch, and the gamma values can be updated as the
    ...
    /**
     * Prepares gamma and the associated theta expectations are initialized so that
     * the iterative updates to them can begin.
     * 
    ...
    /**
     * Performs the main iteration to determine the topic distribution of the given
     * document against the current model parameters. The non zero values of phi
     * will be stored in {@code indexMap} and {@code phiCols}
     * 
     ...
     *                       indices in
     * @param phiCols        the array to store the normalized non zero values of
     *                       phi in, where each value corresponds to the associated
     *                       index in {@code indexMap}
     * @param K              the number of topics
     * @param gamma_d        the initial value of γ that will be altered to the
     *                       topic assignments, but not normalized
    ...
    private void computePhi(final Vec doc, int[] indexMap, double[] phiCols, int K, final Vec gamma_d, final Vec ELogTheta_d, final Vec ExpELogTheta_d) {
        // φ^k_dn ∝ exp{E[logθdk]+E[logβk,wdn ]}, k ∈ {1, . . . ,K}
        /*
         * we have the exp versions of each, and exp(log(x)+log(y)) = x y so we can just
         * use the doc product between the vectors per document to get the normalization
         * constan Z
         * 
         * When we update γ we multiply by the word, so non presnet words have no
         * impact. So we don't need ALL of the columbs from φ, but only the columns for
         * which we have non zero words.
         */

        /*
         * normalized for each topic column (len K) of the words in this doc. We only
         * need to concern oursleves with the non zeros
         * 
         * Beacse we need to update several iterations, we will work with the inernal
         * stricture dirrectly instead of using expensitve get/set on a Sparse Vector
         */
        ...
        // iterate till convergence or we hit arbitrary 100 limit (dont usually see more
        // than 70)
        for (int iter = 0; iter < 100; iter++) {
            ...
            /*
             * //original papser uses a tighter bound, but our approximation isn't that good
             * - and this seems to work well enough 0.01 even seems to work, but need to try
             * that more before switching
             */
    ...
    private void initialize() {
        ...
        // lambda has now been intialized, ELogBeta and ExpELogBeta will be intialized /
        // updated lazily
    }
}
