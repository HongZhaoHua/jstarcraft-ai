...
public abstract class SupportVectorLearner implements Serializable {
    ...
    // Implementation note, NaN is used to indicate a cache value that has not been
    // computed yet.
    @ParameterHolder
    private KernelTrick kernel;
    /**
     * The array of vectors. In the training phase, this should be the set of all
     * training vectors. After training, this should contain only the set of support
     * vectors.
     */
    protected List<Vec> vecs;
    /**
     * The array of coefficients associated with each support vector. This should be
     * instantiated directly when training. When the set of alphas and support
     * vectors is finalized, {@link #setAlphas(double[]) } should be called with a
     * reference to itself or the array where the final alphas are stored. This will
     * initialized any accelerating structures so that
     * {@link #kEvalSum(com.jstarcraft.ai.jsat.linear.Vec) } can be called.
     */
     ...
     * hint, and save that row in this object to avoid overhead of hitting the LRU.
     * See {@link #accessingRow(int) }
     */
    ...
    public enum CacheMode {
        ...
        NONE,
        /**
         * The entire kernel matrix will be created and cached ahead of time. This is
         * the best option if your data set is small and the kernel cache can fit into
         * memory.
         */
        FULL,
        /**
         * Only the most recently used rows of the kernel matrix will be cached (LRU).
         * When a call to {@link #k(int, int) } occurs, the first value will be taken to
         * be the row of the matrix. <br>
         * Because the kernel matrix is symmetric, if a cache miss occurs - the column
         * value will be checked for its existence. If the row is present, it will be
         * used instead. If both rows are not present, then a new row is inserted for
         * the first index, and another row evicted if necessary. <br>
         * The {@link #cacheEvictions} indicates how many times a row has been evicted
         * from the cache. <br>
...
//            this.availableRow = Arrays.copyOf(toCopy.availableRow, toCopy.availableRow.length);
    ...
    public void setCacheSize(long N, long bytes) {
        ...
        {
            // guessing 2 work overhead for object header + one pointer reference to the
            // array, asusming 64 bit
     ...
     * caches. Calling this with the current cache mode will initialize the caches.
     * Once training is complete, call again with {@code null} to deinitialize the
     * caches.
     *
     * @param cacheMode
     */
...
//                    fullCache[i][j-i] = k(i, j);
     ...
     * {@link #setAlphas(double[]) } before calling this, but kernel evaluations may
     * be slower if this is not done.
     * 
     ...
     * kernel values for a specific row. The row may be selected out from the cache
     * into its own location to avoid excess LRU overhead. Giving a negative index
     * indicates that we are done with the row, and removes it. This method may be
     * called multiple times with different row values. But when done accessing a
     * specific row, a negative value should be passed in.
     *
     *
    ...
    /**
     * Internal kernel eval source. Only call directly if you KNOW you will not be
     * re-using the resulting value and intentionally wish to skip the caching
     * system
     *
     * @param a the first vector index
     * @param b the second vector index
