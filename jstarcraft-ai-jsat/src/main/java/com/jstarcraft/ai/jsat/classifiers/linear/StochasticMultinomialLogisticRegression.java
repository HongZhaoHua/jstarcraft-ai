...
public class StochasticMultinomialLogisticRegression implements Classifier, Parameterized, SimpleWeightVectorModel {
     ...
     * @param epochs              the maximum number of training epochs to go
     *                            through
     * @param regularization      the scale factor applied to the regularization
     *                            term
    ...
    /**
     * Creates a new Stochastic Multinomial Logistic Regression that uses a
     * {@link Prior#GAUSSIAN} prior with a regularization scale of 1e-6. It will do
     * at most 50 epochs with a learning rate of 0.1
     */
    ...
    public enum Prior {
        ...
        /**
         * This is the Elastic Net prior, and it uses the extra {@link #setAlpha(double)
         * alpha} parameter. This prior is a mix of both {@link #LAPLACE} and
         * {@link #GAUSSIAN}. Alpha should be in the range [0,1]. Alpha weight will be
         * applied to the Laplace prior, and (1-alpha) weight will be applied to the
         * Gaussian prior. The extreme values of this collapse into the Laplace and
         * Gaussian priors.
         */
        ELASTIC {
         ...
         * extra {@link #setAlpha(double) alpha} parameter. Alpha should be in the range
         * (0, Infty).
         */
        CAUCHY {
    ...
    /**
     * Sets whether or not to learn the bias term for a model. If no bias term is in
     * use, the model learned must pass through the origin of the world. The use of
     * the bias term is very important for low dimensional problems, but less so for
     * many higher dimensional problems.
     * 
     * @param useBias {@code true} if the bias term should be used, {@code false}
     *                otherwise
     */
     ...
     * regularization so that they can not make the coefficients go from positive to
     * negative or negative to positive. If clipping is on, the value will go to
     * zero instead. If off, the value will be allowed to change signs. <br>
    ...
    /**
     * Sets the tolerance that determines when the training stops early because the
     * change has become too insignificant.
     * 
    ...
    /**
     * Sets whether or not to perform implicit standardization of the feature values
     * when performing regularization by the prior. If set on, the input data will
     * be adjusted to have zero mean and unit variance. This is done without
     * destroying sparsity. If there is not regularization, this parameter has no
     * impact.
     * 
    ...
    /**
     * Sets the amount of data points used to form each gradient update. Increasing
     * the batch size can help convergence. By default, a mini batch size of 1 is
     * used.
     * 
    ...
    public void train(ClassificationDataSet dataSet) {
        ...
        if (standardized) {
            ...
            // Now transform it so that stdDevs holds standard deviations, and means is the
            // mean / standDev
     ...
     * number of output classes. Altering the returned vector will alter the model.
     * The i'th index of the vector corresponds to the weight therm for the i'th
     * index in an input.
     * 
