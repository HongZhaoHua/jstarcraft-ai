...
public class ElkanKernelKMeans extends KernelKMeans {
     ...
     * @param new_assignments  the new assignment of data points to cluster centers
     * @param parallel         source of threads for parallel computation. May be
     *                         null.
     */
     ...
     *                   classifications. Should be the same length as the number of
     *                   data points
     * @param exactTotal determines how the objective function (return value) will
     *                   be computed. If true, extra work will be done to compute
     *                   the exact distance from each data point to its cluster. If
     *                   false, an upper bound approximation will be used.
    ...
    private void initialClusterSetUp(final int k, final int N, final double[][] lowerBound, final double[] upperBound, final double[][] centroidSelfDistances, final int[] assignment, boolean parallel) {
        ParallelUtils.run(parallel, N, (start, end) -> {
            // Skip markers
    ...
    private int step4_5_6_distanceMovedBoundsUpdate(final int k, final int N, final double[][] lowerBound, final double[] upperBound, final int[] assignment, final boolean[] r, boolean parallel) {
        final double[] distancesMoved = new double[k];
        // copy the originonal sqrd norms b/c we need them to compute the distance means
        // moved
        final double[] oldSqrdNorms = new double[meanSqrdNorms.length];
        for (int i = 0; i < meanSqrdNorms.length; i++)
            oldSqrdNorms[i] = meanSqrdNorms[i] * normConsts[i];

        // first we need to do assignment movement updated, otherwise the sqrdNorms will
        // be wrong and we will get incorrect values for cluster movemnt
    ...
    private void step3bUpdate(double[] upperBound, final int q, double[][] lowerBound, final int c, double[][] centroidSelfDistances, final int[] assignment, final AtomicBoolean changeOccurred) {
        // 3(b)
