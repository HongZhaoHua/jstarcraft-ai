...
import static com.jstarcraft.ai.jsat.linear.Matrix.OuterProductUpdate;
...
public class HessenbergForm implements Serializable {
    ...
    public static void hess(Matrix A, ExecutorService threadpool) {
        ...
        for (int i = 0; i < m - 2; i++) {
            ...
            // Now re use s to quickly get the norm of vk, since it will be almost the same
            // vector
            s -= vk[i + 1] * vk[i + 1];
            vk[i + 1] -= s1;
            s += vk[i + 1] * vk[i + 1];
            double s1Inv = 1.0 / Math.sqrt(s);// Re use to store the norm of vk. Do the inverse to multiply quickly instead of
                                              // divide
            for (int j = i + 1; j < m; j++)
                vk[j] *= s1Inv;

            // Update sub sub matrix A[i+1:m, i:m]
            // NOTE: The first column that will be altered can be done ourslves, since we
            // know the value set (s1) and that all below it will ber zero
