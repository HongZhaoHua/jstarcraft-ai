...
import com.jstarcraft.ai.jsat.distributions.kernels.KernelPoints;
...
public class KernelSGD implements UpdateableClassifier, UpdateableRegressor, Parameterized {
     ...
     * @param budgetStrategy the budget maintenance strategy to use
     * @param errorTolerance the error tolerance used in certain budget maintenance
     *                       steps
     * @param budgetSize     the maximum support vector budget
     */
     ...
     * {@link #setBudgetStrategy(com.jstarcraft.ai.jsat.distributions.kernels.KernelPoint.BudgetStrategy)
     * budget strategies}
     * 
     ...
     * training. Increasing the budget size will increase the accuracy of the model,
    ...
    /**
     * Sets the base learning rate to start from. Because of the decay rate in use a
     * good value for &eta; is 1.0.
     * 
    ...
    public void update(DataPoint dataPoint, double weight, int targetClass) {
        ...
        if (kpoint != null) {
            ...
            pred.mutableMultiply(-eta_t);// should we wrap in a scaledVec? Probably fine unless someone pulls out a 200
                                         // class problem
