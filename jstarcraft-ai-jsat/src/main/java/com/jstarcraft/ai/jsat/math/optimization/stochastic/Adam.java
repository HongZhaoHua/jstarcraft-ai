package com.jstarcraft.ai.jsat.math.optimization.stochastic;
...
public class Adam implements GradientUpdater {
    ...
    public double update(Vec x, Vec grad, double eta, double bias, double biasGrad) {
        ...
        /*
         * "Note that the efficiency of algorithm 1 can, at the expense of clarity, be
         * improved upon by changing the order of computation, e.g. by replacing the
         * last three lines in the loop with the following line:" θ_t = θ_{t−1} −[α
         * ·√(1−(1−β_2)^t) · (1−(1−β_1)^t)−1] ·m_t/√v_t
         */
        double cnst = eta * alpha * sqrt(1 - pow((1 - beta_2), t)) / (1 - pow((1 - beta_1), t));

        // while the algorithm may converge well with sparse data, m and v are likely to
        // all be non-zero after observing lots of data.
