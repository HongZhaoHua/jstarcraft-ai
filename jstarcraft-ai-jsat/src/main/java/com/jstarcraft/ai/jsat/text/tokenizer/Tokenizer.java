...
public interface Tokenizer extends Serializable {
    /**
     * Breaks the input string into a series of tokens that may be used as features
     * for a classifier. The returned tokens must be either new string objects or
     * interned strings. If a token is returned that is backed by the original
     * document, memory may get leaked by processes consuming the token. <br>
     ...
     * @param storageSpace an already allocated (but empty) list to place the tokens
     *                     into
     */
