...
public class EigenValueDecomposition implements Serializable {
        ...
        while (n >= low) {
            ...
            } else if (l == n - 1) // Two roots found
            {
                ...
                while (m >= l) {
                    ...
                    if (abs(H.get(m, m - 1)) * (abs(q) + abs(r)) < eps * (abs(p) * (abs(H.get(m - 1, m - 1)) + abs(z) + abs(H.get(m + 1, m + 1))))) {
                        break;
                    }
                    m--;
                }
     ...
     * altered. If the input is symmetric, a more efficient algorithm will be used.
     * 
     ...
     * 
     * @param A   the square matrix to work on.
     * @param eps the numerical tolerance for differences in value to be considered
     *            the same.
     */
    public EigenValueDecomposition(Matrix A, double eps) {
        ...
        if (Matrix.isSymmetric(A, eps)) {
            // Would give it the transpose, but the input is symmetric. So its the same
            // thing
     ...
     * associated eigen value. Sorting can not occur if complex values are present.
     * 
    ...
    public void sortByEigenValue(Comparator<Double> cmp) {
        ...
        IndexTable it = new IndexTable(DoubleLists.unmodifiable(DoubleArrayList.wrap(d, d.length)), cmp);
    ...
    /**
     * Returns the raw eigenvector matrix. Modifying this matrix will effect others
     ...
     * @param shift the direction to perform the computation. Either 1 for after the
     ...
     * @param notlast <tt>true<tt> if the 2nd column after <tt>k</tt> should be
     *                updated
    ...
    private void hqr2SolveComplexEigenEquation(final int i, final double p, final double q, final double eps, final double norm, final double w, final double z, final double r, final double ra, final double sa, final double s, final double[] cr, final int n) {
        double x;
        double y;
        double vr;
        double vi;
