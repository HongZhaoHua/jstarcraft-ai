...
import com.jstarcraft.ai.jsat.classifiers.PriorClassifier;
...
public class LVQLLC extends LVQ {
     ...
     * @param dm             the distance metric to use
     * @param iterations     the number of iterations to perform
     * @param localClasifier the classifier to use as a local classifier for each
     *                       prototype
     */
     ...
     * @param dm                      the distance metric to use
     * @param iterations              the number of iterations to perform
     * @param localClasifier          the classifier to use as a local classifier
     *                                for each prototype
    ...
    /**
     * Each prototype will create a classifier that is local to itself, and trained
     * on the points that belong to the prototype and those near the border of the
     * prototype. This sets the classifier that will be used
     * 
    ...
    public CategoricalResults classify(DataPoint data) {
        List<? extends VecPaired<VecPaired<Vec, Integer>, Double>> nns = vc.search(data.getNumericalValues(), 2);
    ...
    public void train(ClassificationDataSet dataSet, boolean parallel) {
        ...
        for (DataPointPair<Integer> dpp : dataSet.getAsDPPList()) {
            ...
            if (Math.min(minDist / minDist2, minDist2 / minDist) > (1 - tmpEps) && Math.max(minDist / minDist2, minDist2 / minDist) < (1 + tmpEps)) {
