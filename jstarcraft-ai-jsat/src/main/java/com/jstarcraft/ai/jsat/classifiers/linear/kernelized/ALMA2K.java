...
public class ALMA2K extends BaseUpdateableClassifier implements BinaryScoreClassifier, Parameterized {
     ...
     * weighted by the number of successful uses of the hypothesis during training.
     * This effectively reduces the variance of the classifier. It has no impact on
     * the training / update phase, only the classification results are impacted.
     * <br>
     * <br>
     * Unlike most algorithms, this can be changed at any time without issue - even
     * after the algorithm has been trained the type of output (averaged or last)
     * can be switched on the fly.
     * 
     * @param averaged {@code true} to use the averaged out, {@code false} to only
     *                 use the last hypothesis
     */
     ...
     * @return the score for the input indicating which side of the hyperplane it is
     *         on
     */
