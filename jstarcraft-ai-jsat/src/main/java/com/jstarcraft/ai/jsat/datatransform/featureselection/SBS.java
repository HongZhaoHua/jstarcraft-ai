...
import static com.jstarcraft.ai.jsat.datatransform.featureselection.SFS.addFeature;
import static com.jstarcraft.ai.jsat.datatransform.featureselection.SFS.removeFeature;
...
public class SBS extends RemoveAttributeTransform {
     ...
     *                    feature subset
     * @param maxDecrease the maximum tolerable increase in the error rate when a
     *                    feature is removed
     */
    ...
    private void search(DataSet dataSet, Object learner, int minFeatures, int maxFeatures, int folds) {
        ...
        while (catSelected.size() + numSelected.size() > minFeatures) {
            ...
            if (SBSRemoveFeature(available, dataSet, catToRemove, numToRemove, catSelected, numSelected, learner, folds, rand, maxFeatures, bestScore, maxDecrease) < 0)
                break;
     ...
     *
     * @param available    the set of available features from [0, n) to consider for
     *                     removal
     ...
     * @param folds        the number of cross validation folds to determine
     *                     performance
     * @param rand         the source of randomness
     * @param maxFeatures  the maximum allowable number of features
     * @param PbestScore   an array to behave as a pointer to the best score seen so
     *                     far
     * @param maxDecrease  the maximum allowable decrease in accuracy from the best
     *                     accuracy we see
    ...
    /**
     * Sets the maximum allowable decrease in accuracy (increase in error) from the
     * previous set of features to the new current set.
     *
    ...
    /**
     * Returns the maximum allowable decrease in accuracy from one set of features
     * to the next
     *
     * @return the maximum allowable decrease in accuracy from one set of features
     *         to the next
     */
