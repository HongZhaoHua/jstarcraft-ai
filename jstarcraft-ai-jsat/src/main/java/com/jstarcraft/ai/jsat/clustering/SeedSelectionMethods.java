...
public class SeedSelectionMethods {
    ...
    static public enum SeedSelection {
         ...
         * The solution is O(log(k)) competitive with the optimal k clustering when
         * using {@link EuclideanDistance}. <br>
         * <br>
        ...
        /**
         * The first seed is chosen randomly, and then all others are chosen to be the
         * farthest away from all other seeds
         */
        FARTHEST_FIRST,

        /**
         * Selects the seeds in one pass by selecting points as evenly distributed
         * quantiles for the distance of each point from the mean of the whole data set.
         * This makes the seed selection deterministic <br>
         * <br>
         * See: J. A. Hartigan and M. A. Wong, "A k-means clustering algorithm", Applied
     ...
     * @param k               the number of seeds to choose
     * @param dm              the distance metric to used when selecting points
     * @param accelCache      the cache of pre-generated acceleration information
     *                        for the distance metric. May be null
     * @param rand            a source of randomness
     * @param selectionMethod The method of seed selection to use.
     * @return a list of the copies of the chosen vectors.
     */
     ...
     *                        perform clustering. {@code false} if it should be done
    ...
    /**
     * Selects seeds from a data set to use for a clustering algorithm. The indices
     * of the chosen points will be placed in the <tt>indices</tt> array.
     * 
     * @param d               the data set to perform select from
     * @param indices         a storage place to note the indices that were chosen
     *                        as seed. The length of the array indicates how many
     *                        seeds to select.
     * @param dm              the distance metric to used when selecting points
     * @param rand            a source of randomness
     * @param selectionMethod The method of seed selection to use.
    ...
    private static void ffSelection(final int[] indices, Random rand, final DataSet d, final int k, final DistanceMetric dm, final List<Double> accelCache, boolean parallel) {
        ...
        for (int j = 1; j < k; j++) {
            ...
            // Atomic integer storres the index of the vector with the current maximum
            // minimum distance to a selected centroid
