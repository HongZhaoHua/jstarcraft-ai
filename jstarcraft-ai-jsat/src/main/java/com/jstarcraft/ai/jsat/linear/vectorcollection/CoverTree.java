 ...
 * Note, this implementation does not yet support parallel construction. <br>
 * 
 * See:
 * <ul>
 ...
 * 97–104). New York: ACM. Retrieved from <a href=
 * "http://www.cs.princeton.edu/courses/archive/spr05/cos598E/bib/covertree.pdf">here</a></li>
...
//            this.root.setLevel(0);
        ...
        {
            /*
             * If the insetion point x has a distance that is SUPER far away, the below
             * bound may never hold. Thus, lets detect loops and short circuit
             */
...
//        maxDistDirty = true;
    ...
    private class TreeNode implements Cloneable, Serializable {
        ...
        /**
         * Copy constructor. Will not copy the parent node. But will copy children nodes
         * and set their parents appropriately.
         * 
         * @param toCopy
         */
        ...
        public void findNN(int k, Vec query, List<Double> x_qi, BoundedSortedList<IndexDistPair> knn) {
            ...
            while (!toEval_stack.isEmpty()) {
                ...
                for (int i_oder = it.length() - 1; i_oder >= 0; i_oder--)// reverse order so stack goes in sorted order
                {
                    ...
                    if (knn.size() < k || knn.last().getDist() > child_query_dist[i] - q.maxdist()) {// Add to the search Q
                        toEval_stack.push(q);
        ...
        // This is the old search code, new code (above) avoids recursion and makes
        // explicit stack
