...
public class FastICA implements InvertibleTransform {
    ...
    /**
     * The FastICA algorithm requires a function f(x) to be used iteratively in the
     * algorithm, but only makes use of the first and second derivatives of the
     * algorithm.
     */
    ...
    public enum DefaultNegEntropyFunc implements NegEntropyFunc {
         ...
         * function is described as a "good general-purpose contrast function" in the
         ...
         * paper may be better than {@link #LOG_COSH} "when the independent components
         * are highly super-Gaussian, or when robustness is very important"
         */
        EXP {
            @Override
            public double deriv1(double x) {
         ...
         * justified on statistical grounds only for estimating sub-Gaussian independent
    ...
    /**
     * Creates a new FastICA transform that will attempt to fit 10 components. This
     * is likely not optimal for any particular dataset
     */
    public FastICA() {
        this(10);
    }
     ...
     *
     * @param data        the data set to transform
     * @param C           the number of base components to assume and try to
     *                    discover
     * @param G           the Negative Entropy function to use
     * @param preWhitened {@code true} to assume the data has already been whitened
     *                    before being given to the transform, {@code false} and the
    ...
    public void fit(DataSet data) {
        ...
        for (int p = 0; p < C; p++) {
            ...
            do {
                ...
                /*
                 * Convergence check at end of loop: "Note that convergencemeans that the old
                 * and new values of w point in the same direction, i.e. their dot-product is
                 * (almost) equal to 1. It is not necessary that the vector converges to a
                 * single point, since w and −w define the same direction"
                 */
    ...
    /**
     * Controls where or not the implementation assumes the input data is already
     * whitened. Whitening is a requirement for the algorithm to work as intended.
     *
     * @param preWhitened {@code true} to assume the data has already been whitened,
     *                    {@code false} for this object to do its own whitening
     */
