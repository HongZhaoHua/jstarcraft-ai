...
package com.jstarcraft.ai.jsat.math.rootfinding;
...
public interface RootFinder extends Serializable {
    /**
     * Attempts to numerical compute the root of a given function, such that
     * f(<tt>args</tt>) = 0. Only one variable may be altered at a time
     * 
     * @param eps            the accuracy desired for the solution
     * @param maxIterations  the maximum number of steps allowed before forcing a
     *                       return of the current solution.
     * @param initialGuesses an array containing the initial guess values
     * @param f              the function to find the root of
     * @param pos            the index of the argument that will be allowed to alter
     *                       in order to find the root. Starts from 0
