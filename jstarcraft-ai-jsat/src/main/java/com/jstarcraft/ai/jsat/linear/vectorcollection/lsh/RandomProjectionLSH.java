...
import com.jstarcraft.ai.jsat.linear.distancemetrics.CosineDistance;
...
public class RandomProjectionLSH<V extends Vec> implements VectorCollection<V> {
     ...
     * bit encoding, we waste anotehr 32-64 bits on object overhead, so store it all
     * in one big array isntead of individual ones. Haming distance is the number of
     * bit differences, and sinx XOR results in a 1 bit only if the bits are not the
     * same, we can use the bit count of the XORed value to count the hamming
     * distance.
     */
     ...
     * distributed values to approximate a full matrix with considerably less memory
     * storage.
     * 
     * @param vecs     the list of vectors to form a collection for
     * @param ints     the number of integers to use for the encoding
     * @param poolSize the number of normally distributed random variables to store.
     *                 Matrix values will be pulled on demand from an index in the
     *                 pool of values.
     */
