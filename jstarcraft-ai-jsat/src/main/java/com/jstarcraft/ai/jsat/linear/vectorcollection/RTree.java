...
import static com.jstarcraft.ai.jsat.linear.VecPaired.extractTrueVec;
...
public class RTree<V extends Vec> implements IncrementalCollection<V> {
     ...
     * 
     * @param query     the rectangle to find all points that would be contained by
     *                  it
    ...
    private RNode cloneChangeContext(RNode toClone) {
        if (toClone != null)
            if (toClone instanceof com.jstarcraft.ai.jsat.linear.vectorcollection.RTree.RNode)
    ...
    private RNode<V> chooseLeaf(Vec v) {
        ...
        while (!N.isLeaf()) {
            /*
             * CL3 [Choose subtree ] If N 1s not a leaf, (1) let F be the entry in N whose
             * rectangle FI needs least enlargement to include EI.
             * 
             * (2) Resolve ties by choosmg the entry with the rectangle of smallest area
             */
            ...
            /*
             * CL4 [Descend until a leaf 1s reached.] Set N to be the cMd node pomted to by
    ...
    private RNode<V> splitNode(RNode<V> toSplit) {
        // Quadratic Split
        /*
         * [Pick first entry for each group ] Apply Algorithm PickSeeds to choose two
         * entries to be the first elements of the groups Assign each to a group
         */
        double d = Double.MIN_VALUE;
        int e1 = 0, e2 = 0;
        // PickSeeds
        /**
         * PSl [Calculate inefficiency of grouping entries together] For each pair of
         * entries El and E2, compose a rectangle J including El I and E2 I Calculate d
         * = area(J) - area(El I) - area(E2 I)
         */
        ...
        if (toSplit.isLeaf()) {
            ...
            while (!toAsign.isEmpty()) {
                /*
                 * QS2 [Check If done ] If all entries have been assigned, stop. If one group
                 * has so few entries that all the rest must be assigned to it in order for it
                 * to have the muumum number m, assign them and stop
                 */
                 ...
                 * entry to assign. Add it to the group whose covering rectangle will have to be
                 * enlarged least to accommodate it. Resolve ties by adding the entry to the
                 * group mth smaller area, then to the one with fewer entries, then to either
                 * Repeat from QS2
                 */

                // PICK NEXT
                /*
                 * [Determme cost of puttmg each entry m each group ] For each entry E not yet m
                 * a group, calculate d1 = the area increase required in the covering rectangle
                 * of Group 1 to include EI. Calculate d2 similarly for Group 2
                 */
        ...
        } else// TODO handles rectangles... very similar,
        {
            ...
            while (!toAsign.isEmpty()) {
                /*
                 * If one group has so few entries that all the rest must be assigned to it m
                 * order for it to have the muumum number m,
                 */
    ...
    private void AdjustTree(RNode<V> L, RNode<V> LL) {
        /*
         * AT1 [Imtlahze.] Set N=L If L was split previously, set NN to be the resultmg
         * second node
         */
        RNode<V> N = L, NN = LL;
        ...
        {
            /*
             * AT3 [Adjust covermg rectangle m parent entry ] Let P be the parent node of N,
             * and let EN be N’s entry in P Adjust EN I so that it tightly encloses all
             * entry rectangles in N.
             */
            RNode<V> P = N.parent;
            ...
            if (NN != null) {
                 ...
                 * earher spht, create a new entry Em with ENNp pointmg to NN and Em I enclosing
                 * all rectangles in NN. Add Em to P If there is room. Otherwise, invoke
                 * SplitNode to produce P and PP containing Em and all P’s old entries
                 */
                if (P.add(NN))
            ...
            /*
             * AT5 [Move up to next level.] Set N=P and set NN=PP If a spht occurred, Repeat
             * from AT2.
             */
            N = P;
        }
    ...
    public synchronized void insert(V v) {
        ...
        allVecs.add(v);
        /*
         * I1 [Find position for new record ] Invoke ChooseLeaf to select a leaf node L
         * in which to place E
         */
         ...
         * Othemse mvoke SplitNode to obtam L and U contammg E and all the old entrees
         * of L
         */
        if (L.add(indx))// true if we need to split
            LL = splitNode(L);
        /*
         * I3 [Propagate changes upward] Invoke AdjustTree on L, also passmg U If a spht
         * was performed
         */
        AdjustTree(L, LL);
    ...
    private double minMaxDist(Vec p, Rectangle r) {
        if (r.contains(p))
            return 0;
        /*
         * MinMaxDist is usualy describe with the minimum over another loop, explicisty
         * as the euclidant distance Instead, we prepare a single vector for each loop
         * (k), and set its values accordinly (with index k being an exception in the
         * value set) We then compute the distance metric and select the min for each k
         */
