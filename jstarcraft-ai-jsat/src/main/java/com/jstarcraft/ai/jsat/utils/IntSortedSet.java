...
public class IntSortedSet extends AbstractSet<Integer> implements Serializable, SortedSet<Integer> {
    ...
    /**
     * more efficient insertion of many items by placing them all into the backing
     * store, and then doing one large sort.
     *
     * @param set
     * @param parallel
     */
    ...
    public boolean add(Integer e) {
        ...
        // checking here means we will actually expand 1 value early, but avoids a
        // double check later
        // so small cost, I'll take it to simplify code
        if (size >= store.length)
            store = Arrays.copyOf(store, Math.max(1, store.length) * 2);

        // fast path for when we are filling a set, and iterating from smallest to
        // largest index.
