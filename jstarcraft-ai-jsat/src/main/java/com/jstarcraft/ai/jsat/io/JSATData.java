 ...
 * reading and writing will usually be significantly faster. <br>
 ...
 * read back in using {@link GZIPInputStream}. <br>
 * 
 * 
 *
 * @author Edward Raff
...
public class JSATData {
    ...
    public static final byte[] MAGIC_NUMBER = new byte[] { 'J', 'S', 'A', 'T', '_', '0', '0' };
    ...
    public static enum FloatStorageMethod {
        ...
        FP32 {
            ...
            @Override
            protected boolean noLoss(double orig) {
                // below can only return true if ther eis no loss in storing these values as 32
                // bit floats instead of doubles
    ...
    /**
     * This method writes out a JSAT dataset to a binary format that can be read in
     * again later, and could be read in other languages.<br>
     * <br>
     * The format that is used will understand both {@link ClassificationDataSet}
     * and {@link RegressionDataSet} datasets as special cases, and will store the
     * target values in the binary file. When read back in, they can be returned as
     * their original dataset type, or treated as normal fields as a
     * {@link SimpleDataSet}.<br>
     * The storage format chosen for floating point values will chose a method that
     * results in no loss of precision when reading the data back in.
     *
     * @param         <Type>
     ...
     * @param outRaw  the raw output stream, the caller should provide a buffered
     *                stream.
     * @param fpStore the storage method of storing floating point values, which may
     *                result in a loss of precision depending on the method chosen.
     * @throws IOException
     */
     ...
     * datapoints into the given output stream. This works in a thread safe manner.
     * 
     ...
     * @param predicting information on the class label, may be {@code null} if not
     *                   a classification dataset
     * @param fpStore    the format floating point values should be stored as
     * @param type       what type of data set (simple, classification, regression)
     *                   to be written
        ...
        return new DataWriter(out, catInfo, dim, type) {
            ...
            protected void pointToBytes(double weight, DataPoint dp, double label, ByteArrayOutputStream byteOut) {
                try {
                    ...
                    if (type == DataWriter.DataSetType.REGRESSION) {
                        /*
                         * if dense, we only need to just add the extra double. If sparse, we do the
                         * index and then the double.
                         */
                        if (numericVals.isSparse())
    ...
    /**
     * This loads a JSAT dataset from an input stream, and will not do any of its
     * own buffering. The DataSet will be returned as either a
     * {@link SimpleDataSet}, {@link ClassificationDataSet}, or
     * {@link RegressionDataSet} depending on what type of dataset was originally
     * written out.<br>
     *
    ...
    /**
     * Loads in a JSAT dataset as a {@link ClassificationDataSet}. An exception will
     * be thrown if the original dataset in the file was not a
     * {@link ClassificationDataSet}.
     *
     * @param inRaw the input stream, caller should buffer it
     * @return a ClassificationDataSet object
     * @throws IOException
     * @throws ClassCastException if the original dataset was a not a
    ...
    /**
     * Loads in a JSAT dataset as a {@link RegressionDataSet}. An exception will be
     * thrown if the original dataset in the file was not a
     ...
     * written out.<br>
     * <br>
     * This method supports forcing the load to return a {@link SimpleDataSet}. <br>
     ...
     *                        {@link SimpleDataSet}, otherwise it will be determined
     *                        based on the input streams contents.
     * @return a dataset
     * @throws IOException
     */
     ...
     *                        {@link SimpleDataSet}, otherwise it will be determined
     *                        based on the input streams contents.
     * @param store           the backing mechanism to store all the data in and use
     *                        for the returned dataset object
     * @return a dataset
     * @throws IOException
     */
    @SuppressWarnings("unchecked")
