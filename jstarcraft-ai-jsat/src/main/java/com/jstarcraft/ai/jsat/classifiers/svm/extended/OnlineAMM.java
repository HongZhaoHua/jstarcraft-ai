...
public class OnlineAMM extends BaseUpdateableClassifier implements Parameterized {
     ...
     * how to handle re-assignment of data points to weight vectors. Also allows us
     * to handle removed cases by checking if our owner is in the map. Use nextID to
     * make sure we give every new vec a unique ID with respect to the class label
     */
    ...
    /**
     * Sets the pruning constant which controls how powerful pruning is when pruning
     * occurs. Increasing C increases how many weights will be pruned. Changes to
     * the scaling of feature vectors may require a change in the value of C <br>
    ...
    /**
     * Performs the work for an update. It can be used for regular online learning,
     * or in the batch scenario where the assignments should not be updated with
     * each online update. <br>
     ...
     * @param z_t       the hyperplane of the true class with the maximum response,
     *                  or {@link Integer#MIN_VALUE} if it should be calculated
    ...
    protected int update(DataPoint dataPoint, final int y_t, int z_t) {
        ...
        /*
         * Note, we use the same code for both online and batch AMM. If the input is the
         * miniumum value, we are doing a normal online update. If not, we use the given
         * input & need to find out the response for the specified z_t instead
         * 
         * Not clear in the paper, how do we handle the case when z_t was assigned to
         * someone removed? Lets just treat it as unknown. Should be rare, so we wont
         * worry about values changing and being reassigned. Especially since weight
         * vectors near the front should be stable (they survided longer than those
         * formerly infront of them after all).
         */
        double z_t_val;
        ...
        int i_t = (y_t > 0 ? 0 : 1);// j_t may be implicit, but i_t needs to belong to someone in the event of a
                                    // tie. So just give it to the first class that isn't y_t
        double i_t_val = 0.0;
        int j_t = -1;
        ...
        for (int i = 0; i < weightMatrix.size(); i++) {
            ...
            // Also must check for implicit weight vectors needing an update (making them
            // non-implicit)
        ...
        if (time % k == 0)// Pruning time!
        {
            ...
            IntArrayList classOwner = new IntArrayList(weightMatrix.size());
            IntArrayList vecID = new IntArrayList(weightMatrix.size());
