 ...
 * {@link #dist(com.jstarcraft.ai.jsat.linear.Vec, com.jstarcraft.ai.jsat.linear.Vec) } using the list and indices.
...
public interface DistanceMetric extends Cloneable, Serializable {
     ...
     * and there for, more similar, the vectors are. 0 indicates the vectors are the
     * same.
     * 
    ...
    /**
     * Returns true if this distance metric obeys the rule that, for any x, y, and z
     * &isin; S <br>
     * d(x, y) = d(y, x)
     * 
     ...
     * 
     * @return true if this distance metric supports the triangle inequality, false
     *         if it does not.
     */
    ...
    /**
     * Returns true if this distance method obeys all the rules required to be a
     * valid metric.
     * 
     * @return true if this distance method obeys all the rules required to be a
     *         valid metric.
     */
    ...
    /**
     * All metrics must return values greater than or equal to 0. The upper bound on
     * the value returned is different for different metrics. This method returns
     * the theoretical maximal value that could be returned by this distance metric.
     * That means {@link Double#POSITIVE_INFINITY } is a valid return value.
     * 
     ...
     * {@code true}, then a cache can be obtained from this distance metric and used
     * in conjunction with
     * {@link #dist(int, com.jstarcraft.ai.jsat.linear.Vec, java.util.List, java.util.List) } and
     * {@link #dist(int, int, java.util.List, java.util.List) } to perform distance
     * computations.
     * 
    ...
    /**
     * Returns a cache of double values associated with the given list of vectors in
     * the given order. This can be used by the distance metric to increase runtime
     * at the cost of memory. This is an optional method. <br>
    ...
    /**
     * Computes the distance between 2 vectors in the original list of vectors. <br>
     * If the cache input is {@code null}, then
     * {@link #dist(com.jstarcraft.ai.jsat.linear.Vec, com.jstarcraft.ai.jsat.linear.Vec) } will be called directly.
     * 
    ...
    /**
     * Computes the distance between one vector in the original list of vectors with
     * that of another vector not from the original list. <br>
     * If the cache input is {@code null}, then
     * {@link #dist(com.jstarcraft.ai.jsat.linear.Vec, com.jstarcraft.ai.jsat.linear.Vec) } will be called directly.
     * 
     ...
     * large number of distance computations are going to be done against points in
    ...
    /**
     * Computes the distance between one vector in the original list of vectors with
     * that of another vector not from the original list, but had information
     * generated by {@link #getQueryInfo(com.jstarcraft.ai.jsat.linear.Vec) }. <br>
     * If the cache input is {@code null}, then
     * {@link #dist(com.jstarcraft.ai.jsat.linear.Vec, com.jstarcraft.ai.jsat.linear.Vec) } will be called directly.
     * 
     * @param a     the index of the vector in the cache
