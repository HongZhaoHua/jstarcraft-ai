...
import com.jstarcraft.ai.jsat.datatransform.InPlaceTransform;
...
import com.jstarcraft.ai.jsat.linear.MatrixOfVecs;
import com.jstarcraft.ai.jsat.linear.MatrixStatistics;
import com.jstarcraft.ai.jsat.linear.SparseMatrix;
...
public abstract class DataSet<Type extends DataSet> {
    ...
    /**
     * Creates a new dataset containing the given datapoints. The number of features
     * and categorical data information will be obtained from the DataStore.
     *
     ...
     *
     * @param numerical  the number of numerical features for points in this dataset
     * @param categories the information and number of categorical features in this
     *                   dataset
     */
     ...
     * points. Changing this may be beneficial for expert users who know how their
     * data will be accessed, or need to make modifications for more efficient
     * storage.<br>
     * If the currently data store is not empty, it's contents will be copied to the
     * new store. <br>
    ...
    /**
     * 
     * @return {@code true} if row-major traversal should be the preferred iteration
     *         order for this data store, or {@code false} if column-major should be
     *         preferred.
     */
    ...
    /**
     * Sets the unique name associated with the <tt>i</tt>'th numeric attribute. All
     * strings will be converted to lower case first.
     * 
     * @param name the name to use
     * @param i    the <tt>i</tt>th attribute.
     * @return <tt>true</tt> if the value was set, <tt>false</tt> if it was not set
     *         because an invalid index was given .
     */
     ...
     * each data point with the new value. No mutation of the data points will occur
     * 
     ...
     * replacing each data point with the new value. No mutation of the data points
     * will occur.
     * 
     * @param dt       the transformation to apply
     ...
     * transform supports mutating the original data points, this will be applied if
     * {@code mutableTransform} is set to {@code true}
     *
     * @param dt     the transformation to apply
    ...
    /**
     * Applies the given transformation to all points in this data set in parallel.
     * If the transform supports mutating the original data points, this will be
    ...
    public void applyTransformMutate(final DataTransform dt, boolean mutate, boolean parallel) {
        if (mutate && dt instanceof InPlaceTransform) {
            ...
            ParallelUtils.range(size(), parallel).forEach(i -> ipt.mutableTransform(getDataPoint(i)));
        } else
     ...
     * object from the given list. All vecs in the given list must be of the same
     * size.
     *
    ...
    /**
     * Returns the <tt>i</tt>'th data point in this set. The order will never chance
     * so long as no data points are added or removed from the set.
     * 
     ...
     * stable and uses more memory. <br>
     * NaNs / missing values will be ignored in the statistics for each column.
     * 
     * @param useWeights {@code true} to return the weighted statistics, unweighted
     *                   otherwise.
    ...
    public OnLineStatistics[] getOnlineColumnStats(boolean useWeights) {
        ...
        /**
         * We got to skip nans, count their weight in each column so that we can still
         * fast count zeros
         */
     ...
     * proportion of each data point contains non zero numerical values. A mean of 1
     * indicates all values were fully dense, and a mean of 0 indicates all values
     ...
     * 
     * @param indicies the indices of data points to insert into the new dataset,
     *                 and will be placed in the order listed.
     * @return a new dataset that is a specified subset of this dataset, and backed
     *         by the same values
     */
    abstract protected Type getSubset(List<Integer> indicies);

    /**
     * This method returns a dataset that is a subset of this dataset, where only
     * the rows that have no missing values are kept. The new dataset is backed by
     * this dataset.
     *
     ...
     * @param splits any array, where the length is the number of datasets to create
     *               and the value of in each index is the fraction of samples that
     *               should be placed into that dataset. The sum of values must be
     ...
     * data points in each set will be random. These are meant for cross validation
     * 
     * @param folds the number of cross validation sets to create. Should be greater
     *              then 1
     ...
     * copies, in the same order as this data set. However, altering this list will
     * have no effect on DataSet. Altering the DataPoints in the list will effect
    ...
    /**
     * The data set can be seen as a NxM matrix, were each row is a data point, and
     * each column the values for a particular variable. This method grabs all the
     * numerical values for a 'column' and returns it as one vector. <br>
     * This vector can be altered and will not effect any of the values in the data
     * set
     * 
     ...
     * columns without mutating the data points themselves. However, future callers
     * may or may not receive the same vector objects.
     * 
     ...
     * {@link #getNumericColumn(int) } when multiple columns are needed. <br>
     * <br>
     * A set of columns to skip can be provided in order to save memory if one does
     * not need all the columns. <br>
     * <br>
     * Note, that the columns returned by this method may be cached and re used by
     * the DataSet itself. If you need to alter the columns you should create your
     * own copy of these vectors. If you know that you will be the only person
     ...
     * may or may not receive the same vector objects.
     * 
     * @param skipColumns if a column's index is in this set, a {@code null} will be
     *                    returned in the array at the column's index instead of a
     *                    vector
     * 
     * @return an array of the column vectors
     */
     ...
     * This matrix can be altered and will not effect any of the values in the data
     * set.
     * 
    ...
    /**
     * Creates a matrix backed by the data set, where each row is a data point from
     * the dataset, and each column is one of the numeric examples from the data
     * set. <br>
     * Any modifications to this matrix will be reflected in the dataset. <br>
     * This method has the advantage over {@link #getDataMatrix() } in that it does
     * not use any additional memory and it maintains any sparsity information.
     * 
    ...
    /**
     * Returns a new version of this data set that is of the same type, and contains
     * a different list pointing to the same data points.
     * 
    ...
    /**
     * Returns a new version of this data set that is of the same type, and contains
     * a different listing pointing to shallow data point copies. Because the data
     * point object contains the weight itself, the weight is not shared - while the
     * vector and array information is. This allows altering the weights of the data
     * points while preserving the original weights. <br>
    ...
    /**
     * This method returns the weight of each data point in a single Vector. When
     * all data points have the same weight, this will return a vector that uses
     * fixed memory instead of allocating a full double backed array.
     *
