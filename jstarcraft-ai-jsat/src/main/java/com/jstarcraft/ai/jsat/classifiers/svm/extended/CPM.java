 ...
 *
 * <br>
 * See: Kantchelian, A., Tschantz, M. C., Huang, L., Bartlett, P. L., Joseph, A.
 * D., & Tygar, J. D. (2014). Large-margin Convex Polytope Machine. In
...
public class CPM implements BinaryScoreClassifier, Classifier, Parameterized {
    ...
    /**
     * Creates a new CPM classifier, with default parameters that should work well
     * for most cases.
     */
    public CPM() {
        this(1.0);
     ...
     * @param lambda           the regularization parameter
     * @param K                the number of hyper-planes to learn with.
     * @param entropyThreshold the parameter that encourages non-linearity to be
     *                         exploited
     */
     ...
     * hyper-planes are used, where larger values encourage using more of the hyper
     * planes.<br>
     * <br>
     * This method is adjusted from the paper's definition so that the input can be
     * any non-negative value. It is recommended to try values in the range of [0,
     * 10]
     *
     ...
     * model complexity. This value is adjusted from the form in the original paper
     * so that you do not need to consider the number of epochs explicitly. The
     * effective regularization will be divided by the total number of training
     * updates.
     *
    ...
    /**
     * Sets the number of hyper planes to use when training. A normal linear model
     * is equivalent to using only 1 hyper plane. The more hyper planes used, the
     * more modeling capacity the algorithm has, but the slower it will run.
     *
    ...
    public CategoricalResults classify(DataPoint data) {
        ...
        if (neg_score > 0 && pos_score > 0)// ambigious case, lets go with larger magnitude
        {
            ...
            cr.setProb(1, 1.0);
        else if (neg_score > pos_score)// not actually how describes in paper, but its ambigious - so lets use larger
                                       // to tie break
    ...
    /**
     * 
     * @param dots                        dot product between the input and each of
     *                                    the k hyper planes
     * @param owned                       a count of how many data points are
     *                                    assigned to this hyper plane
     * @param assignments                 maps each data point to the hyper plane
     *                                    that owns it. May have negative values for
     *                                    points not yet assigned
     * @param assigned_positive_instances the number of <bold>positive</bold>
     *                                    instances taht have been assigned to a
     *                                    hyper plane
     */
...
//                    cur_entropy += -p_i * Math.log(p_i)/Math.log(2);
        ...
        int k_inc_max = 0;
        if (old_owner >= 0)// don't need to compute entropy, moving to any position that owns fewer would
                           // increase entropy
        {
            for (int i = 1; i < dots.length(); i++)
     ...
     *                 vector.
     * @param sign_mul Either positive or negative 1. Controls whether or not the
...
//                W.mutableMultiply(1-eta*lambda);
    ...
    /**
     * Provides a distribution of reasonable values for the
     * {@link #setEntropyThreshold(double) } parameter
     *
     * @param d the dataset to get the guess for
     * @return the distribution to search this parameter from
     */
