...
public class StochasticGradientBoosting implements Regressor, Parameterized {
    ...
    /**
     * The proportion of the data set to be used for each iteration of training. The
     * points that make up the iteration are a random sampling without replacement.
     */
     ...
     * @param weakLearner     the weak learner to fit to the residuals in each
     *                        iteration
     ...
     * @param trainingPortion the proportion of the data set to use for each
    ...
    /**
     * Sets the learning rate of the algorithm. The GB version uses a learning rate
     * of 1. SGB uses a learning rate in (0,1) to avoid overfitting. The learning
     * rate is multiplied by the output of each weak learner to reduce its
     * contribution.
     * 
     * @param learningRate the multiplier to apply to the weak learners
     ...
     * fraction of the data set at each iteration in order to reduce overfitting and
     * add randomness.
     * 
     * @param trainingProportion the fraction of training the data set to use for
     *                           each iteration of SGB
     * @throws ArithmeticException if the trainingPortion is not a valid fraction in
     *                             (0, 1]
     */
    ...
    public void train(RegressionDataSet dataSet, boolean parallel) {
        // use getAsDPPList to get coppies of the data points, so we can safely alter
        // this set
     ...
     * @param h                 the regressor that is having the error of its output
     *                          minimized
    ...
    private double getMinimizingErrorConst(final RegressionDataSet backingResidsList, final Regressor h) {
        // Find the coeficent that minimized the residual error by finding the zero of
        // its derivative (local minima)
     ...
     * error of the Regressor as a function of the constant factor multiplied on the
     * Regressor's output.
     * 
     * @param backingResidsList the DataPointPair list of residuals
     * @param h                 the regressor that is having the error of its output
    ...
    private Function1D getDerivativeFunc(final RegressionDataSet backingResidsList, final Regressor h) {
        final Function1D fhPrime = (double x) -> {
             ...
             * f(x)-f(x-eps)
             *
             * hEst is the output of the new regressor, target is the true residual target
             * value
             *
             * So we have several (hEst_i c1 - target)^2 - (hEst_i c2 -target)^2 //4 muls, 3
             * subs Where c2 = c1-eps Which simplifies to (c1 - c2) hEst ((c1 + c2) hEst - 2
             * target) = eps hEst (c1Pc2 hEst - 2 target)//3 muls, 1 sub, 1 shift (mul by 2)
             *
             * because eps is on the outside and independent of each individual summation,
             * we can move it out and do the eps multiplicatio ont he final result. Reducing
             * us to
             *
             ...
             * per loop
             *
             * Which reduce computation, and allows us to get the result in one pass of the
             * data
             */
