...
public class DenseMatrix extends GenericMatrix {
     ...
     * 
     * @param a the first Vector, this new Matrix will have as many rows as the
     *          length of this vector
     * @param b the second Vector, this new Matrix will have as many columns as this
     *          length of this vector
     */
    ...
    public void multiply(Vec b, double z, Vec c) {
        ...
        for (int i = 0; i < rows(); i++) {
            // The Dense construcure does not clone the matrix, it just takes the refernce
            // -making it fast
            DenseVector row = new DenseVector(matrix[i]);
            c.increment(i, row.dot(b) * z);// We use the dot product in this way so that if the incoming matrix is sparce,
                                           // we can take advantage of save computaitons
        }
    }
     ...
     * @return vkNorm plus the summation of the squared values for all values copied
     *         into vk
     */
    ...
    private void qrUpdateQ(DenseMatrix Q, int k, double[] vk, double TwoOverBeta) {
        ...
        // We are computing Q' in what we are treating as the column major order, which
        // represents Q in row major order, which is what we want!
    ...
    private void qrUpdateR(int k, int N, DenseMatrix A, double[] vk, double TwoOverBeta, int M) {
        // First run of loop removed, as it will be setting zeros. More accurate to just
        // set them ourselves
        if (k < N) {
    ...
    public void transposeMultiply(final Matrix b, final Matrix C, ExecutorService threadPool) {
        ...
        // We only want to take care of the case where everything is of this class. Else
        // let the generic version handle quirks
    ...
    public void multiply(Matrix b, Matrix C) {
        ...
        // We only want to opt the case where everyone is dense, else - let the generic
        // version handle quierks
         ...
         * Pract. Exper. 2004; 16:799–815 (DOI: 10.1002/cpe.793)
         * 
         */
    ...
    public void multiply(Matrix b, Matrix C, ExecutorService threadPool) {
        ...
        if (this.rows() / NB2 >= LogicalCores)// Perform block execution only when we have a large enough matrix to keep ever
                                              // core busy!
        {
    ...
    public Matrix[] lup() {
        ...
        // Initalization is a little wierd b/c we want to handle rectangular cases as
        // well!
        if (rows() > cols())// In this case, we will be changing U before returning it (have to make it
                            // smaller, but we can still avoid allocating extra space
            L = new DenseMatrix(rows(), cols());
        else
            L = new DenseMatrix(rows(), rows());

        for (int i = 0; i < U.rows(); i++) {
            // If rectangular, we still need to loop through to update ther est of L - even
            // though we wont make many other changes
            if (i < U.cols()) {
                // Partial pivoting, find the largest value in this colum and move it to the
                // top!
        ...
        {
            // We need to change U to a square nxn matrix in this case, we can safely drop
            // the last 2 rows!
            double[][] newU = new double[cols()][];
            System.arraycopy(U.matrix, 0, newU, 0, newU.length);
            U = new DenseMatrix(newU);// We have made U point at a new object, but the array is still pointing at the
                                      // same rows!
        }
    ...
    private class LUProwRun implements Callable<Integer> {
        ...
        /**
         * Returns the index of the row with the largest absolute value we ever saw in
         * column k+1
         */
        public Integer call() throws Exception {
            for (int i = k + 1 + threadNumber; i < U.rows(); i += LogicalCores) {
                ...
                // We perform the first iteration of the loop outside, as we want to cache its
                // value for searching later
