...
public class PearsonDistance implements DistanceMetric {
    ...
    /**
     * Creates a new standard Pearson Distance that does not ignore zero values and
     * anti-correlated values are considered far away.
     */
     ...
     *                         "missing" or "no vote", and will not contribute. But
     *                         this will not change the mean value used.
     *                         {@code false} produces the standard Pearson value.
     * @param absoluteDistance {@code true} to use the absolute correlation, meaning
     *                         correlated and anti-correlated values will have the
     *                         same distance.
     */
     ...
     * is all zeros, the result is undefined. In cases where both are zero vectors,
     * 1 will be returned to indicate they are the same. In cases where one of the
     * numerator coefficients is zero, its value will be bumped up to an epsilon to
     * provide a near result. <br>
     * <br>
     ...
     * @param bothNonZero {@code false} is the normal Pearson correlation.
     *                    {@code true} will make the computation ignore all indexes
     *                    where one of the values is zero, the mean will be from all
     *                    non zero values in each vector.
    ...
    /*
     * TODO Accerlation for Pearson can be done, its a little complicated (you cache
     * the means and Sqrd values - so that you can do just 1 pass over all values).
     * But thats a good bit of code, and the above needs to be cleaned up before
     * implementing that.
     */
