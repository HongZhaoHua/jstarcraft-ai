...
public class ClassificationDataSet extends DataSet<ClassificationDataSet> {
     ...
     * values. Pairing is determined by the iteration order of each collection.<br>
     * It is assumed that all options for the target variable are contained in the
     * given targets list.
     *
     *
     ...
     * @param categories the information about each categorical variable in the
     *                   problem.
     * @param predicting the information about the target class
     */
     ...
     * @return a combination of all the data sets in <tt>list</tt> except the one at
     *         index <tt>exception</tt>
     */
     ...
     * @throws IndexOutOfBoundsException if <tt>i</tt> is not a valid index into the
     *                                   data set.
     */
    ...
    /**
     * This method is a counter part to {@link #getNumericColumn(int) }. Instead of
     * returning all values for a given attribute, all values for the attribute that
     * are members of a specific class are returned.
     * 
     * @param category the category desired
    ...
    /**
     * Returns the data set as a list of {@link DataPointPair}. Each data point is
     * paired with it's true class value, which is stored in a double. Altering the
     * data points will effect the data set. Altering the list will not. <br>
     * The list of data points will come in the same order they would be retrieved
     * in using {@link #getDataPoint(int) }
     * 
