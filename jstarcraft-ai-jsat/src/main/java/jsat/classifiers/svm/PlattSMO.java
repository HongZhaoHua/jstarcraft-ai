 ...
 * "http://www.bradblock.com/Sequential_Minimal_Optimization_A_Fast_Algorithm_for_Training_Support_Vector_Machine.pdf">here</a></li>
...
public class PlattSMO extends SupportVectorLearner implements BinaryScoreClassifier, WarmRegressor, Parameterized, WarmClassifier {
    ...
    private boolean[] I0;
    /**
     * Indicates if the value that is currently in I0 is also in I0_a. If its not in
     * I0, the value in this array is false
     */
    private boolean[] I0_a;
    /**
     * Indicates if the value that is currently in I0 is also in I0_b. If its not in
     * I0, the value in this array is false
     */
        ...
        if (warmSolution instanceof PlattSMO || warmSolution instanceof BinaryScoreClassifier) {
            ...
            WarmScope: // We need to use one of the methods to detemrine if the model is a fit
            {
                if (warmSolution instanceof PlattSMO) {
                    // if this SMO object was learend on the same data, we can get a very good guess
                    // on C
                    PlattSMO warmSMO = (PlattSMO) warmSolution;

                    // first, we need to make sure we were actually trained on the same data
                    // TODO find a better way to ensure this is true, it is POSSIBLE that we could
                    // have same labels and different data
                ...
                if (warmSolution instanceof BinaryScoreClassifier) {
                    ...
                    for (int i = 0; i < vecs.size(); i++) {
                        // get the loss, normaly wrapped by max(x, 0), but that will be handled by the
                        // clamp
...
//        sparsify();
    ...
    private void updateSetR(int i, double C) {
        /**
         * See page 5 of he pseudo code paper version of "Improvements to the SMO
    ...
    protected boolean takeStep(int i1, int i2) {
        ...
        // "Update fcache[i] for i in I_0 using new Lagrange multipliers", done inside
        // loop check for new bounds
        for (int i = 0; i < I0.length; i++) {
            if (!I0[i])
                continue;
            if (i != i1 && i != i2)
        ...
        while (!finished)// occurs at most 3 times
        {
            if (!case1 && (alpha1 > 0 || (alpha1_S == 0 && deltaPhi > 0)) && (alpha2 > 0 || (alpha2_S == 0 && deltaPhi < 0))) {
                ...
                if (L < H) {
                    ...
                    finished = true;
                case1 = true;
            } else if (!case2 && (alpha1 > 0 || (alpha1_S == 0 && deltaPhi > 2 * epsilon)) && (alpha2_S > 0 || (alpha2 == 0 && deltaPhi > 2 * epsilon))) {
                // compute L, H, (wrt. alpha1, alpha2*)
                L = max(0, -gamma);
                H = min(C2, -gamma + C1);
                if (L < H) {
                    ...
                    finished = true;
                case2 = true;
            } else if (!case3 && (alpha1_S > 0 || (alpha1 == 0 && deltaPhi < -2 * epsilon)) && (alpha2 > 0 || (alpha2_S == 0 && deltaPhi < -2 * epsilon))) {
                // compute L, H, (wrt. alpha1*, alpha2)
                L = max(0, gamma);
                H = min(C2, C1 + gamma);
                if (L < H) {
                    ...
                    finished = true;
                case3 = true;
            } else if (!case4 && (alpha1_S > 0 || (alpha1 == 0 && deltaPhi < 0)) && (alpha2_S > 0 || (alpha2 == 0 && deltaPhi > 0))) {
                // compute L, H, (wrt. alpha1*, alpha2*)
                L = max(0, -gamma - C1);
                H = min(C2, -gamma);
                if (L < H) {
    ...
    /**
     * Updates the threshold for regression based off of "using only i1, i2, and
     * indices in I_0"
     * 
    ...
    /**
     * Returns the local decision function for regression training purposes without
     * the bias term
     * 
     * @param v the index of the point to select
     * @return the decision function output sans bias
     ...
     * evaluations.
     * 
     * @param modificationOne {@code true} to us modificaiotn one, {@code false} to
     *                        use modification two.
     */
     ...
     * This variable has no impact during classification problems. For regression
     * problems, any predicated value that is within the epsilon of the target will
     * be treated as "correct". Increasing epsilon usually decreases the number of
     * support vectors, but may reduce the accuracy of the model
     * 
    ...
    public void train(RegressionDataSet dataSet, Regressor warmSolution) {
        ...
        if (warmSolution != null) {
            /*
             * warm for regression is kinda hard, so we use it to set the initial focus on a
             * few data poitns. We set the alpha values to be non zero for the points that
             * are errors and let everything else past the "margin" (ie: in the cone of the
             * espilon) be zero. Then the first few passes of SMO will optimize this intial
             * set, and then when examineAll becomes true larger corrections can be made.
             */
            examinAll = false;
...
//                err = signum(err)*min(abs(err), 1);
            ...
            for (int i = 0; i < N; i++) {
                ...
                updateSetR(i, C_i);
                // fix up the bounds on bias term, see eq (3) in "Improvements to the SMO
                // algorithm for SVM regression."
                if (I0[i] || I1[i] || I3[i])
                    b_up = min(b_up, F_i);
                if (I0[i] || I1[i] || I2[i])
