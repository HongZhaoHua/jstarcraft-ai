...
public class DANN implements Classifier, Parameterized {
     ...
     * Creates a new DANN classifier
     * 
     * @param kn the number of neighbors to use in casting a net to build a new
     *           metric
     * @param k  the number of neighbors to use with the final metric in
     *           classification
     */
     ...
     * Creates a new DANN classifier
     * 
     * @param kn            the number of neighbors to use in casting a net to build
     *                      a new metric
     * @param k             the number of neighbors to use with the final metric in
     *                      classification
     * @param eps           the regularization to add to the metric creation
     ...
     *                      each classification
     * @param vcf           the default vector collection that will be used for
     *                      initial neighbor search
     */
     ...
     * query. The metric should converge quickly. For this reason, and do to a lack
     * of performance difference, it is highly recommended to use the default of 1
     * iteration.
     * 
    ...
    /**
     * Sets the regularization to apply the the diagonal of the scatter matrix when
     * creating each new metric.
     * 
    ...
    public CategoricalResults classify(DataPoint data) {
        ...
        // TODO threadlocal DoubleList / DenseVec might be better in practice for memory
        // use
        ...
        for (int iter = 0; iter < maxIterations; iter++) {
            ...
            List<? extends VecPaired<VecPaired<Vec, Integer>, Double>> vecs = (iter == 0) ? vc.search(query, kn) : brute(query, sigma, kn);
        ...
        List<? extends VecPaired<? extends VecPaired<Vec, Integer>, Double>> knn = brute(query, sigma, k);
    ...
    private List<? extends VecPaired<VecPaired<Vec, Integer>, Double>> brute(Vec query, Matrix sigma, int num) {
        ...
        BoundedSortedList<VecPairedComparable<VecPaired<Vec, Integer>, Double>> knn = new BoundedSortedList<>(num, num);
