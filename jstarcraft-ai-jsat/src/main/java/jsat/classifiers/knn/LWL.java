...
public class LWL implements Classifier, Regressor, Parameterized {
     ...
     * @param dm         the metric to use when selecting the nearest points to a
     *                   query
     */
     ...
     *                   query
     * @param kf         the kernel function used to weight the local points
     * @param vcf        the factory to create vector collections for storing the
     *                   points
     */
    ...
    public CategoricalResults classify(DataPoint data) {
        ...
        List<? extends VecPaired<VecPaired<Vec, Double>, Double>> knn = vc.search(data.getNumericalValues(), k);
    ...
    private List<VecPaired<Vec, Double>> getVecList(ClassificationDataSet dataSet) {
        List<VecPaired<Vec, Double>> trainList = new ArrayList<>(dataSet.size());
        for (int i = 0; i < dataSet.size(); i++)
            trainList.add(new VecPaired<>(dataSet.getDataPoint(i).getNumericalValues(), new Double(dataSet.getDataPointCategory(i))));
        return trainList;
    }
    ...
    private List<VecPaired<Vec, Double>> getVecList(RegressionDataSet dataSet) {
        List<VecPaired<Vec, Double>> trainList = new ArrayList<>(dataSet.size());
        for (int i = 0; i < dataSet.size(); i++)
            trainList.add(new VecPaired<>(dataSet.getDataPoint(i).getNumericalValues(), dataSet.getTargetValue(i)));
        return trainList;
    }
