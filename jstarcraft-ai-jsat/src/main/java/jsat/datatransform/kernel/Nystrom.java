...
public class Nystrom extends DataTransformBase {
    ...
    public enum SamplingMethod {
        ...
        UNIFORM,
        /**
         * Selects the basis vectors by a weighted probability of the kernel value of
         * k(x<sub>i</sub>, x<sub>i</sub>) for each <i>i</i>. If a kernel returns 1 for
         * all k(x<sub>i</sub>, x<sub>i</sub>), then this reduces the uniform sampling.
         * Takes O(n) time
         */
        DIAGONAL,
        /**
         * Selects the basis vectors by the weighted probability of the column norms of
         * the gram matrix for each vector. Takes O(n<sup>2</sup>) time
         */
        NORM,
        /**
         * Selects the basis vectors as the means of a k-means clustering. Takes the
         * time needed to perform k-means
         */
        KMEANS,
    }
    ...
    /**
     * Creates a new Nystrom approximation object using the {@link RBFKernel RBF
     * Kernel} with 500 basis vectors
     *
     */
    public Nystrom() {
     ...
     * 
     * @param k                    the kernel trick to form an approximation of
     * @param basisSize            the number of basis vectors to use, this is the
     *                             output dimension size.
     * @param method               what sampling method should be used to select the
     *                             basis vectors from the full data set.
     * @param ridge                a non negative additive term to regularize the
     *                             eigen values of the decomposition.
     * @param sampleWithReplacment {@code true} if the basis vectors should be
     *                             sampled with replacement, {@code false} if they
     *                             should not.
     */
     ...
     * 
     * @param k                    the kernel trick to form an approximation of
     * @param dataset              the data set to form the approximate feature
     *                             space from
     * @param basisSize            the number of basis vectors to use, this is the
     *                             output dimension size.
     * @param method               what sampling method should be used to select the
    ...
    /**
     * Performs sampling of a data set for a subset of the vectors that make a good
     * set of basis vectors for forming an approximation of a full kernel space.
     * While these methods are motivated from Nystrom's algorithm, they are also
     * useful for others.
     * 
     ...
     * matrix. This can be particularly useful when using a large (500+) number of
     * components.
     *
