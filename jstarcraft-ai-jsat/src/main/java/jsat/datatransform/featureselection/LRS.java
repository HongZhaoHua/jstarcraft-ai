...
public class LRS implements DataTransform {
     ...
     * @param L         the number of features to greedily add
     * @param R         the number of features to greedily remove
     * @param evaluater the regressor to use in determining accuracy given a feature
     *                  subset
     * @param folds     the number of cross validation folds to use in selection
     */
    ...
    private void search(DataSet cds, int L, int R, Object evaluater, int folds) {
        ...
        if (L > R) {
            ...
            // Select L features
            for (int i = 0; i < L; i++)
                SFS.SFSSelectFeature(available, cds, catToRemove, numToRemove, catSelected, numSelected, evaluater, folds, rand, pBestScore, L);
            ...
            for (int i = 0; i < R; i++)
                SBS.SBSRemoveFeature(available, cds, catToRemove, numToRemove, catSelected, numSelected, evaluater, folds, rand, L - R, pBestScore, 0.0);
        } else if (L < R) {
            ...
            // Remove R features
            for (int i = 0; i < R; i++)
                SBS.SBSRemoveFeature(available, cds, catToRemove, numToRemove, catSelected, numSelected, evaluater, folds, rand, nF - R, pBestScore, 0.0);
            ...
            for (int i = 0; i < L; i++)
                SFS.SFSSelectFeature(available, cds, catToRemove, numToRemove, catSelected, numSelected, evaluater, folds, rand, pBestScore, R - L);
        }
