...
public class Kolmogorov extends ContinuousDistribution {
    ...
    public double cdf(double x) {
        if (x < 0)
            ...
            return 0;
        else if (x >= 5)// By this point, floating point isnt accurate enough to distinguish between 1.0
                        // and the true value.
            return 1;
         ...
         * 
         * Each formula converges very rapidly, interface 3 terms to full IEEE precision
         * for one or the other - crossover point is 1.18 according to Numerical
         * Recipies, 3rd Edition p(334-335)
         */
        double tmp = 0;
        double x2 = x * x;
        if (x < 1.18) {
