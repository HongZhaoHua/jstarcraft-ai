...
public class FibHeap<T> {
     ...
     * containing the minimum key. When a Fibonacci heap H is empty, H:min is NIL.
     */
    FibNode<T> min;
    /**
    ...
    public FibHeap() {
         ...
         * returns the Fibonacci heap object H, where H:n D 0 and H:min D NIL; there are
         * no trees in H.
         */
        this.min = null;
        this.n = 0;
...
//            H.add(x);
    ...
    private void consolidate() {
        ...
        do {
            ...
            runner = runner.right;
        } while (runner != startingNode);// Yes, intentionally comparing objects. Circular link list. So once we reach
                                         // the starting object we are done
...
//                    w = w.left;//otherwise w will start looping through the child list isntead of the root list
        ...
        if (z != null) {
            ...
            if (z.child != null) {
                ...
                do {
                    ...
                    // go to the next item in the list
                    x = x.right;
                } while (x != startingNode);// Yes, intentionally comparing objects. Circular link list. So once we reach
                                            // the starting object we are done
    ...
    public static class FibNode<T> {
        ...
        /**
         * The boolean-valued attribute x:mark indicates whether node x has lost a child
         * since the last time x was made the child of another node. Newly created nodes
         * are unmarked, and a node x becomes unmarked whenever it is made the child of
         * another node.
         */
        boolean mark = false;
         ...
         * {@link #p} value for {@code x} will be set automatically. The {@link #degree}
         * of this node will not be adjusted, and must be incremented correctly by the
         * caller<br>
         *
         * @param x the node to add as a child of this node. Should be a singular item
         */
     ...
     *         {@code null} if a was its own list (ie: a list of size 1, or no
     *         "list").
     */
    private static <T> FibNode<T> delink(FibNode<T> a) {
        ...
        // fix a's links to point to itself not that it has been de-linked from everyone
        // else
        a.left = a.right = a;
    ...
    /**
     * Merges the two lists given by the two nodes. Works if either node represents
     * a list of size 1 / is on its own. The node with the smaller value will be
     * returned.
     * 
     * @param   <T>
     * @param a
    ...
    private static <T> FibNode<T> merge(FibNode<T> a, FibNode<T> b) {
        if (a == b)// handles a and b == null (returns null) or a and b are the same object (return
                   // the obj, don't make weird cycles
            return a;
        if (a == null)
            return b;
        if (b == null)
        ...
        // else, two different non-null nodes, make a the smaller priority one so we
        // always return smallest priority
        if (a.key > b.key) {
            FibNode<T> tmp = a;
            a = b;
            b = tmp;
        ...
        // linked list insert. Indertion used since links can point to themselves if
        // list is empty
