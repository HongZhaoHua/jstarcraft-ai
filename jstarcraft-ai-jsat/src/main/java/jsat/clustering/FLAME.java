...
public class FLAME extends ClustererBase implements Parameterized {
    ...
    /**
     * Sets the maximum number of iterations to perform. FLAME can require far more
     * iterations to converge than necessary to get the same hard clustering result.
     * 
     * @param maxIterations the maximum number of iterations to perform
     */
     ...
     * Negative values are allowed to force all iterations to occur
     * 
    ...
    /**
     * Sets the number of standard deviations away from the mean density a candidate
     * outlier must be to be confirmed as an outlier.
     * 
     * @param stndDevs the number of standard deviations away from the mean density
     *                 an outlier must be
     */
     ...
     * The nearest neighbor only needs to be done once for each point, so the
     * collection should be faster than the naive method when considering both
    ...
    public int[] cluster(DataSet dataSet, boolean parallel, int[] designations) {
        ...
        // NOTE: Density is done in reverse, so large values indicate low density, small
        // values indiciate high density.
        ...
        for (int i = 0; i < fuzzy.length; i++) {
            ...
            if (pos == -1)// TODO how di this happen? Mark it as an outlier. Somehow your whole row became
                          // zeros to cause this
                pos = CSOs.size();
            clusterCounts[pos]++;
        ...
        /*
         * Transform clusterCOunts to indicate the new cluster ID. If everyone gets
         * there own id, no clusters removed. Else, people with a negative value know
         * they need to remove themsleves
         */
        int newCCount = 0;
