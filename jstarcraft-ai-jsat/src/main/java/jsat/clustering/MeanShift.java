...
public class MeanShift implements Clusterer {
    ...
    /**
     * Sets the value by which the bandwidth of the {@link MultivariateKDE} will be
     * scaled by.
     * 
     * @param scaleBandwidthFactor the value to scale bandwidth by
     * @throws ArithmeticException if the value given is {@link Double#NaN NaN } or
    ...
    private void assignmentStep(boolean[] converged, Vec[] xit, int[] designations) {
        // We now repurpose the 'converged' array to indicate if the point has not yet
        // been asigned to a cluster
    ...
    private void mainLoop(final boolean[] converged, final Vec[] xit, final int[] designations, final KernelFunction k, boolean parallel) throws InterruptedException, BrokenBarrierException {
        ...
        int count = 0;
        /*
         * +1 b/c we have to wait for the worker threads, but we also want this calling
         * thread to wait with them. Hence, +1
         */
    ...
    /**
     * Computes the meanShift of the point at the given index, and then updates the
     * vector array to indicate the movement towards the mode for the data point.
     * 
    ...
    private void convergenceStep(final Vec[] xit, int i, final boolean[] converged, final int[] designations, final Vec scratch, final KernelFunction k) {
        ...
        if (contrib.size() == 1) {
            // If a point has no neighbors, it can not shift, and is its own mdoe - so we
            // mark it noise
            converged[i] = true;
            designations[i] = -1;
        } else {
