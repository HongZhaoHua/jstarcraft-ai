...
public class OPTICS extends ClustererBase implements Parameterized {
    ...
    /**
     * The objects contained in OrderSeeds are sorted by their reachability-distance
     * to the closest core object from which they have been directly density
     * reachable.
     * 
    ...
    public enum ExtractionMethod {
        ...
        XI_STEEP_ORIGINAL,
        /**
         * Forms clusters in the reachability plot by drawing a line across it, and
         * using the separations to mark clusters
         */
        THRESHHOLD,
        /**
         * Forms clusters in the reachability plot by drawing a line across it, and
         * using the separations to mark clusters. It then de-noises points by checking
         * their nearest neighbors for consensus
         */
        THRESHHOLD_FIXUP
    }
    ...
    /**
     * Sets the minimum number of points needed to compute the core distance. Higher
     * values tend to smooth out the reachability plot.
     * 
    ...
    /**
     * Sets the {@link VectorCollection} used to produce acceleration structures for
     * the OPTICS computation.
     * 
    ...
    private int threshHoldFixExtractCluster(List<Integer> orderedFile, int[] designations) {
        ...
        for (int i = 0; i < orderedFile.size(); i++) {
            if (designations[i] != NOISE)
                continue;
            // Check if all the neighbors have a consensus on the cluster class (ignoring
            // noise)
    ...
    private int threshHoldExtractCluster(List<Integer> orderedFile, int[] designations) {
        ...
        for (int i = 0; i < orderedFile.size(); i++) {
            ...
            // Climb up to the top of the hill, everything we climbed over is part of the
            // cluster
...
//                r_dists[objIndex] = new_r_dist;
    ...
    /**
     * Returns a copy of the reachability array in correct reachability order. Some
     * values that are not density reachability could have a value of
