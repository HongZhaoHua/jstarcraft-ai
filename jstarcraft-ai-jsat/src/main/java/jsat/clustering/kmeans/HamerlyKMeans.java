 ...
 * Conference on Data Mining (SDM) (pp. 130–140). Retrieved from <a href=
 * "http://72.32.205.185/proceedings/datamining/2010/dm10_012_hamerlyg.pdf">here</a></li>
 ...
 * Mathematics. <a href=
 * "http://doi.org/10.1137/1.9781611974348.37">http://doi.org/10.1137/1.9781611974348.37</a></li>
 * </ul>
 * 
 * @author Edward Raff
 */
public class HamerlyKMeans extends KMeans {
    ...
    protected double cluster(final DataSet dataSet, List<Double> accelCache, final int k, final List<Vec> means, final int[] assignment, final boolean exactTotal, boolean parallel, boolean returnError, Vec dataPointWeights) {
        ...
        /**
         * Used in bound updates. Contains the centroid means from the previous
         * iteration
         */
        ...
        /**
         * lower bound on the distance between x(i) and its second closest center – that
         * is, the closest center to x(i) that is not c(a(i)) <br>
         * denoted l(i)
         */
        ...
        while (updates > 0) {
            ...
            /**
             * we maintain m(ci), which is the radius of a hypersphere centered at centroid
             * ci that contains all points assigned to ci. (Note that m(ci) is easily
             * obtained as the maximum upper-bound of all points in the cluster.)
             */
            ...
            // Algorithm 3, new bounds update scheme. See "Geometric methods to accelerate
            // k-means algorithms"
    ...
    private void EnhancedUpdateBounds(final List<Vec> means1, final double[] distanceMoved, double[] m, final double[] s, final Vec[] oldMeans, final Vec[] tmpVecs, final Vec[] tmpVecs2, double[] updateB, final double[] p, final int[] assignment, final double[] u, final double[] l) {
        // NOTE: special here c'j eans the new cluster location. Only for algorithm 3
        // update. Rest of code uses different notation
        // Paper uses current and next, but we are coding current and previous
        // so in the paper's termonology, c'j would be the current mean, and cj the
        // previous
        for (int i = 0; i < means1.size(); i++) {
            // 3: update←−∞
            double update = Double.NEGATIVE_INFINITY;
            // 4: for each cj in centroids that fulfill (3.10) in decreasing order of ||c'j
            // −cj|| do.
            ...
            for (int order = 0; order < c_order.length(); order++) {
                ...
                if (j == i)
                    continue;
                // check (3.10)
                if (2 * m[j] + s[j] < distanceMoved[j])// paper uses s(c_i) for half the distance, but our code uses it for the full
                                                       // distance
                ...
                double algo2_1_out;
                // begin Algorithm 2 Algorithm for update of l(x, cj) in the multidimensional
                // case, where c(x) = ci
                // 3: t← eq (3.6)
...
//                        double dist = oldMeans[j].add(means.get(j).subtract(oldMeans[j]).multiply(t)).subtract(oldMeans[i]).pNorm(2);
                ...
                // 8: return update calculated by Algorithm 1 (using r and ci = (cix, ciy))
                // multiplied by ||c'j−cj||/2
                // Algorithm 1 Algorithm for update of l(x, cj) in the simplified two
                // dimensional case, where c(x) = ci.
        ...
        // "The appropriate place to calculate the maximum upper bound is before any
        // centroids move", ok we can do upper bound now
     ...
     * @return 0 if no changes in assignment were made, 1 if a change in assignment
     *         was made
     */
    private int mainLoopWork(DataSet dataSet, int i, double[] s, int[] assignment, double[] u, double[] l, AtomicDoubleArray q, Vec[] deltas, final List<Vec> X, final List<Double> distAccel, final List<Vec> means, final List<List<Double>> meanQI, final Vec W) {
     ...
     * @param distanceMoved distance each cluster has moved from the previous
     *                      locations. Updated by this method call.
