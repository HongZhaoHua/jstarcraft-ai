...
import com.jstarcraft.ai.jsat.linear.vectorcollection.RandomBallCover;
            ...
            for (int iters = 0; iters < 10; iters++)
                for (int neighbours : new int[] { 1, 2, 4, 10, 20 }) {
                    ...
                    for (int i = 0; i < foundTrue.size(); i++) {
                        assertTrue(factory.getClass().getName() + " failed " + (i + 1) + "'th / " + neighbours + " " + foundTrue.get(i).pNormDist(2, foundTest0.get(i)), foundTrue.get(i).equals(foundTest0.get(i), 1e-13));
                    }

                    assertEquals(factory.getClass().getName() + " failed " + neighbours, foundTrue.size(), foundTest1.size());
                    for (int i = 0; i < foundTrue.size(); i++) {
                        assertTrue(factory.getClass().getName() + " failed " + (i + 1) + "'th / " + neighbours + " " + foundTrue.get(i).pNormDist(2, foundTest1.get(i)), foundTrue.get(i).equals(foundTest1.get(i), 1e-13));
                    }
