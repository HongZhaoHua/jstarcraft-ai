...
import com.jstarcraft.ai.jsat.classifiers.linear.LinearTools;
import com.jstarcraft.ai.jsat.classifiers.linear.SDCA;
...
import com.jstarcraft.ai.jsat.linear.*;
import com.jstarcraft.ai.jsat.lossfunctions.*;
...
//        for (int round = 0; round < 100; round++)
        {
            for (int attempts = 5; attempts >= 0; attempts--) {
                ...
                /**
                 * B/c of the what SDCA works, it has trouble picking just 1 of perfectly
                 * correlated features. So we will make a 2nd version of the dataset which has 1
                 * pure strong feature, 2 weak features with noise, and 3 weak features.
                 */
                ...
                for (LossC loss : new LossC[] { new LogisticLoss(), new HingeLoss() }) {
                    ...
                    {// SDCA requires a bit more searching b/c it behaved differently than normal
                     // coordinate descent solvers when selecting features
                        do {
                    ...
                    sdca.train(data);
                    w = sdca.getRawWeight(0);
                    if ((int) Math.signum(w.get(3)) != 1 && attempts > 0)// model probablly still right, but got a bad epsilon solution... try again
                                                                         // please!
                    {
                        continue;
                    }
