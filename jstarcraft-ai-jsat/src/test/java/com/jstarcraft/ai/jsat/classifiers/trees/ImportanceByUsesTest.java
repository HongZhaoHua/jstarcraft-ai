...
import com.jstarcraft.ai.jsat.classifiers.trees.ImportanceByUses;
import com.jstarcraft.ai.jsat.classifiers.trees.TreePruner;
...
//            train.applyTransformMutate(new PCA(train, 2, 0));
            ...
            for (int i = good_featres; i < importances.length; i++) {
                for (int j = 0; j < good_featres; j++)
                    if (importances[j] == 0)// sometimes it happens b/c we can seperate on just the first var when
                                            // discretized
                        assertTrue(importances[j] >= importances[i]);
                    else
                        assertTrue(importances[j] > importances[i]);
