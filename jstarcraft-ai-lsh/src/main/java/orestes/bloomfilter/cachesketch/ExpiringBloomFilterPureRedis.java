...
public class ExpiringBloomFilterPureRedis extends AbstractExpiringBloomFilterRedis<String> {
    ...
    public synchronized boolean onExpire() {
        ...
        try (Jedis jedis = pool.getResource()) {
            long expiredItems = (long) jedis.evalsha(expireQueueScript, 3,
                    // Keys:
    ...
    public TimeMap<String> getExpirationMap() {
        try (Jedis jedis = pool.getResource()) {
            return jedis.zrangeWithScores(keys.EXPIRATION_QUEUE_KEY.getBytes(), 0, -1).stream().collect(TimeMap.collectMillis(tuple -> msgPack.decodeItem(tuple.getBinaryElement()), tuple -> (long) tuple.getScore()));
        }
    }
     ...
     * 
     * @param shouldNotCancel Whether the last job is about to end or needs to be
     *                        canceled
    ...
    /**
     * Returns the delay in milliseconds when to next check the queue for expired
     * items.
     *
