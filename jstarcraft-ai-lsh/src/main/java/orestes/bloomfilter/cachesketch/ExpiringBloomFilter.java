...
/**
 * Tracks a mapping from objects to expirations and a Bloom filter of objects
 * that are automatically removed after expiration(obj).
 *
 */
public interface ExpiringBloomFilter<T> extends CountingBloomFilter<T>, TimeToLiveMapAware<T>, ExpirationMapAware<T>, MigratableBloomFilter<T> {
    ...
    /**
     * Determines whether each of the given input elements is known and returns a
     * list of boolean values where each Boolean represents whether or not the
     * corresponding element in the input list is known.
     *
     * An object is known if it is cached or was removed from the cache not for
     * longer than the grace period.
     *
     * @param elements elements to check (or their ids)
     * @return a list of boolean values indicating whether each of the input
     *         elements is known (<code>true</code>) or not (<code>false</code>)
     */
    ...
    /**
     * Reports a write on an object, adding it to the underlying Bloom filter for
     * the remaining ttl
     *
     * @param element the element (or its id)
     * @param unit    the time unit of the returned ttl
     * @return the remaining TTL, if the object was still cached, else
     *         <code>null</code>
     */
     ...
     * @return List of booleans. <code>true</code>, if the elements needs
     *         invalidation
     */
    ...
    /**
     * Reports a write on a list of objects, adding it to the underlying Bloom
     * filter for the remaining ttl
     *
     * @param elements the elements (or its ids)
     * @param unit     the time unit of the returned ttl
     * @return the list of remaining TTLs, if a object was still cached, else
     *         <code>null</code>
     */
    ...
    /**
     * Executes a soft clear operation on the expiring Bloom filter. Clears the
     * bloom filter without deleting any TTLs
     */
    void softClear();
}
