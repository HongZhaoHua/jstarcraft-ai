...
/**
 * Uses regular key-value pairs for counting instead of a bitarray. This
 * introduces a space overhead but allows distribution of keys, thus increasing
 * throughput. Pipelining can also be leveraged in this approach to minimize
 * network latency.
 *
...
public class CountingBloomFilterRedis<T> implements CountingBloomFilter<T>, MigratableBloomFilter<T> {
    ...
    /**
     * Adds all elements to the Bloom filter and returns the estimated count of the
     * inserted elements.
     *
     * @param elements The elements to add.
     * @return An estimation of how often the respective elements are in the Bloom
     *         filter.
     */
    private List<Long> addAndEstimateCountRaw(Collection<T> elements) {
        ...
        // Walk through result in blocks of #hashes and skip the return values from
        // set-bit calls.
        // Get the minimum count for each added element to estimate how often the
        // element is in the Bloom filter.
    ...
    private Map<Integer, Long> getCounts(Jedis jedis, int... positions) {
        ...
        return IntStream.range(0, positions.length).collect(HashMap::new, (m, i) -> m.put(positions[i], (values.get(i) == null) ? 0L : RedisUtils.decodeValue(values.get(i))), Map::putAll);
    }
