...
/**
 * A persistent bloom filter backed by the Redis key value store. Internally it
 * operates on the <i>setbit</i> and <i>getbit</i> operations of Redis. If you
 * need to remove elements from the bloom filter, please use a counting bloom
 * filter, e.g. {@link CountingBloomFilterRedis}. The performance of this data
 * structure is very good, as operations are grouped into fast transactions,
 * minimizing the network overhead of all bloom filter operations to one round
 * trip to Redis.
 *
