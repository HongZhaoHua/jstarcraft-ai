...
public class FilterBuilder implements Cloneable, Serializable {
    ...
    /**
     * Constructs a new Bloom Filter Builder by specifying the expected size of the
     * filter and the tolerable false positive probability. The size of the BLoom
     * filter in in bits and the optimal number of hash functions will be inferred
     * from this.
     *
    ...
    /**
     * Constructs a new Bloom Filter Builder using the specified size in bits and
     * the specified number of hash functions.
     *
    ...
    /**
     * Sets the number of expected elements. In combination with the tolerable false
     * positive probability, this is used to infer the optimal size and optimal
     * number of hash functions of the filter.
     *
    ...
    /**
     * Sets the tolerable false positive probability. In combination with the number
     * of expected elements, this is used to infer the optimal size and optimal
     * number of hash functions of the filter.
     *
     ...
     * For non-counting Bloom filters this setting has no effect.
     * <p>
     * <b>Default</b>: 16
     * </p>
     *
    ...
    /**
     * Sets the name of the Bloom filter. If a redis-backed Bloom filter with the
     * provided name exists and it is compatible to this FilterBuilder
     * configuration, it will be loaded and used. This behaviour can be changed by
     * {@link #overwriteIfExists(boolean)}.
     * <p>
     * <b>Default</b>: ""
     * </p>
     *
    ...
    /**
     * Sets whether any existing Bloom filter with same name should be overwritten
     * in Redis.
     * <p>
     * <b>Default</b>: <tt>false</tt>
     * </p>
     *
     * @param overwrite boolean indicating whether to overwrite any existing filter
     *                  with the same name
     * @return the modified FilterBuilder (fluent interface)
     */
    ...
    /**
     * Adds a read slave to speed up reading access (e.g. contains or
     * getEstimatedCount) to normal and counting Redis-backed Bloom filters. The
     * read slave has to be a slave of the main Redis instance (this can be done in
     * the redis-cli using the SLAVEOF command). This setting might cause stale
     * reads since Redis replication is asynchronous. However anecdotally, in our
     * experiments, we were unable to read any stale data - the replication lag
     * between both Redis instances was small than one round-trip time to Redis.
     *
    ...
    /**
     * Sets the method used to generate hash values. Possible hash methods are
     * documented in the corresponding enum {@link HashProvider.HashMethod}.
     * <p>
     * <b>Default</b>: MD5
     * </p>
     * <p>
     * For the generation of hash values the String representation of objects is
     * used.
     *
     ...
     *
     * @param cleanupInterval The cleanup interval to set, in the provided time
     *                        unit.
    ...
    /**
     * Constructs a Bloom filter using the specified parameters and computing
     * missing parameters if possible (e.g. the optimal Bloom filter bit size).
     *
    ...
    /**
     * Constructs a Counting Bloom filter using the specified parameters and by
     * computing missing parameters if possible (e.g. the optimal Bloom filter bit
     * size).
     *
    ...
    /**
     * @return The number of bits used for counting in case of a counting Bloom
     *         filter
     */
    ...
    /**
     * @return the number of connections used by the Redis Server backing the Bloom
     *         filter
     */
    ...
    /**
     * @return Return the default Charset used for conversion of String values into
     *         byte arrays used for hashing
     */
    ...
    /**
     * @return {@code true} if the Bloom filter that is to be built should overwrite
     *         any existing Bloom filter with the same name
     */
    ...
    /**
     * @return return the list of all read slaves to be used by the Redis-backed
     *         Bloom filter
     */
    ...
    /**
     * Checks whether a configuration is compatible to another configuration based
     * on the size of the Bloom filter and its hash functions.
     *
    ...
    public RedisPool pool() {
        if (done && pool == null) {
            pool = RedisPool.builder().host(redisHost()).port(redisPort()).readSlaves(getReadSlaves()).password(password()).database(database()).redisConnections(redisConnections()).build();
        }
        return pool;
    }
}
