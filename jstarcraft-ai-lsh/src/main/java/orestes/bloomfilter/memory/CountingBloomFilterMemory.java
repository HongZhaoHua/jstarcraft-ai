...
public class CountingBloomFilterMemory<T> implements CountingBloomFilter<T>, MigratableBloomFilter<T> {
    ...
    /**
     * Decrements the internal counter upon deletion and unsets the Bloom filter bit
     * if necessary.
     *
    ...
    protected long decrement(int index) {
        ...
        for (int i = (high - 1); i >= low; i--) {
            if (!decremented) {
                // Flip every bit until you reach the first bit that is one. Flip that and stop
                // flipping.
                if (counts.get(i)) {
                    counts.set(i, false);
                    decremented = true;
