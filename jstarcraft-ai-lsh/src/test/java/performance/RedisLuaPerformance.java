...
public class RedisLuaPerformance {
    ...
    private static void printResult(String label, Snapshot snapshot) {
        String format = String.format(Locale.ENGLISH, "('%s', %.4f, %.4f, %.4f, %.4f, %.4f),", label, snapshot.getMin() / 1e6d, snapshot.getValue(0.25) / 1e6d, snapshot.getMedian() / 1e6d, snapshot.getValue(0.75) / 1e6d, snapshot.getMax() / 1e6d);
    ...
    public RedisLuaPerformance(int items, int runs) {
        ...
        FilterBuilder builder = new FilterBuilder(1_000_000, 0.02).hashFunction(HashProvider.HashMethod.Murmur3).redisBacked(true).redisHost("127.0.0.1").redisPort(6379).redisConnections(10).overwriteIfExists(true).complete();
