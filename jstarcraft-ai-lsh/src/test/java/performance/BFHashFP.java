...
public class BFHashFP {
    ...
    public static void testFalsePositives(int inserts, double p, Randoms mode) {
        BloomFilter<String> dummy = new FilterBuilder().expectedElements(inserts).falsePositiveProbability(p).buildBloomFilter();
        System.out.println(dummy);
        System.out.println("<table><tr><th>Hash function</th><th>Speed  (ms)</th><th>f during insert (%)</th><th>f final (%)</th></tr>");
        // String test = "False Positives, hashData = " + mode.getDescription() + ",
        // inserts = " + inserts + ", size = " + m + ", hashes = " + k + ", Expected
        // final FP-Rate = " + dummy.getFalsePositiveProbability(inserts) * 100;
    ...
    public static void testFP(HashMethod hm, Randoms mode, int n, double p) {
        FilterBuilder builder = new FilterBuilder().expectedElements(n).falsePositiveProbability(p).hashFunction(hm);
        ...
        // String result = "HashMethod : " + hm + ", speed = " + speed + " ms,
        // False-Positives = " + fps + ", FP-Rate = " + fpRate + ", final FP-Rate = " +
        // totalFpRate;
