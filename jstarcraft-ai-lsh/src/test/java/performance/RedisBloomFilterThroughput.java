...
public class RedisBloomFilterThroughput {
    ...
    public static void main(String[] args) throws Exception {
        ...
        FilterBuilder builder = new FilterBuilder(m, k).hashFunction(HashMethod.Murmur3).name("purity").redisBacked(true).redisHost("127.0.0.1").redisPort(6379).redisConnections(10).overwriteIfExists(true).complete();
    ...
    public CompletableFuture<Boolean> testPerformance(FilterBuilder builder, Class<? extends AbstractExpiringBloomFilterRedis> type) {
        List<ExpiringBloomFilter<String>> servers = IntStream.range(0, SERVERS).mapToObj(i -> createBloomFilter(i, builder, type)).collect(toList());
    ...
    private Stream<ScheduledFuture<?>> startUsers(ExpiringBloomFilter<String> server) {
        return IntStream.range(0, USERS_PER_SERVER).mapToObj(userId -> {
            ...
            ScheduledFuture<?> writeProcess = executor.scheduleAtFixedRate(() -> doReportWrite(server), randomDelay, WRITE_PERIOD, TimeUnit.MILLISECONDS);

            // read Bloom filter periodically
            ScheduledFuture<?> bloomFilterReadProcess = executor.scheduleAtFixedRate(() -> doReadBloomFilter(server), randomDelay, READ_PERIOD, TimeUnit.MILLISECONDS);
...
//            servers.forEach(server -> server.remove());
    ...
    private void dumpHistogram(String name, Histogram histogram) {
        ...
        String format = String.format(Locale.ENGLISH, "'%s', %.4f, %.4f, %.4f, %.4f, %.4f", testName + " " + name, snapshot.getMin() / 1e6d, snapshot.getValue(0.25) / 1e6d, snapshot.getMedian() / 1e6d, snapshot.getValue(0.75) / 1e6d, snapshot.getMax() / 1e6d);
