...
public class BloomFilterMigrationThroughput {
    ...
    public static void main(String[] args) throws Exception {
        ...
        FilterBuilder builder = new FilterBuilder(ITEMS, 0.02).hashFunction(HashMethod.Murmur3).name("purity").redisBacked(true).redisHost("127.0.0.1").redisPort(6379).redisConnections(10).overwriteIfExists(true).complete();
    ...
    public void testPerformance(FilterBuilder builder, Class<? extends ExpiringBloomFilter> type) {
        ...
        if (toServer)
            addNewItems(inMemoryFilter, ITEMS);
        LOG.debug("Created in-memory Bloom filter");

        List<ExpiringBloomFilter<String>> servers = IntStream.range(0, SERVERS).mapToObj(i -> createBloomFilter(builder, type)).collect(toList());
        if (!toServer)
            addNewItems(servers.get(0), ITEMS);
    ...
    private void endTest(List<ExpiringBloomFilter<String>> servers, long startTime) {
        ...
        writer.println(String.format(Locale.ENGLISH, "'%s',%.4f,%.4f,%.4f,%.4f,%.4f", testName, (snapshot.getMin() / 1e6d), (snapshot.getValue(.25) / 1e6d), (snapshot.getValue(.5) / 1e6d), (snapshot.getValue(.75) / 1e6d), (snapshot.getMax() / 1e6d)));
