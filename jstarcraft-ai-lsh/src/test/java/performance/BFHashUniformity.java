...
public class BFHashUniformity {
    ...
    public static void testHashDistribution(int hashesPerRound, int rounds, int m, int k, double alpha, StringBuilder log, Randoms mode) {

        List<List<byte[]>> hashData = mode.generate(hashesPerRound, rounds);

        double crit = criticalVal(m, alpha);
        // System.out.println("The critical Chi-Squared-Value for alpha = " + alpha + "
        // is " + crit);
        ...
        String config = "hashes = " + hashesPerRound + ", size = " + m + ", hashes = " + k + ", rounds = " + rounds + ", hashData = " + dataSource;

        log.append("data" + hashesPerRound + "h" + m + "size" + k + "hashes" + " = { " + chiMathematica + "};\n");
        log.append("crit" + hashesPerRound + "h" + m + "size" + k + "hashes" + " = " + crit + ";\n");
        log.append("show[data" + hashesPerRound + "h" + m + "size" + k + "hashes" + ", crit" + hashesPerRound + "h" + m + "size" + k + "hashes" + ", \"Chi-Squared Statistic\\n" + config + "\"]\n");
        ...
        log.append("show[pdata" + hashesPerRound + "h" + m + "size" + k + "hashes" + ", pcrit" + hashesPerRound + "h" + m + "size" + k + "hashes" + ", \"p-Value\\n" + config + "\"]\n");

    }

    public static void testDistribution(HashMethod hm, int hashesPerRound, int m, int k, int rounds, List<List<byte[]>> hashData, StringBuilder chi, StringBuilder pvalue) {
        ...
        String result = "HashMethod : " + hm + ", " + " hashes = " + hashesPerRound + ", size = " + m + ", hashes = " + k + ", rounds = " + rounds;
    ...
    public static enum Randoms {
        UUIDS("UUIDs", i -> UUID.randomUUID().toString()), INTS("increasing integers", i -> i + ""), ID("incresing String IDs", i -> "object" + i), RANDINTS("random integers", i -> new Random(i).nextInt() + ""), RANDWORDS("random words", i -> RandomStringUtils.random(new Random(i).nextInt(100))), BYTES("increasing bytes", i -> {
