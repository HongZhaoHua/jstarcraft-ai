...
public class BloomFilterMagnus<E> implements Serializable {
     ...
     * be countingBits*expectedElements.
     *
     ...
     * (hashes) is estimated from the total size of the Bloom and the number of
     * expected elements.
     *
     * @param bitSetSize              defines how many bits should be used in total
     *                                for the filter.
    ...
    public BloomFilterMagnus(double falsePositiveProbability, int expectedNumberOfElements) {
        this(Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2))) / Math.log(2), // countingBits = hashes / ln(2)
                expectedNumberOfElements, (int) Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2)))); // hashes = ceil(-log_2(false prob.))
    }
     ...
     *                                       for the filter.
     * @param expectedNumberOfFilterElements defines the maximum number of elements
    ...
    /**
     * Calculates the expected probability of false positives based on the number of
     * expected filter elements and the size of the Bloom filter. <br />
     * <br />
     * The value returned by this method is the <i>expected</i> rate of false
     * positives, assuming the number of inserted elements equals the number of
     * expected elements. If the number of elements in the Bloom filter is less than
     * the expected value, the true probability of false positives will be lower.
     *
     ...
     * Use getFalsePositiveProbability() to calculate the probability of this being
     * correct.
     *
     ...
     * @return true if all the elements in countingBits could have been inserted
    ...
    /**
     * Returns the expected number of elements to be inserted into the filter. This
     * value is the same value as the one passed to the constructor.
     *
