...
public class MemoryBFTest {
    ...
    public void serializeBloomFilter() throws IOException, ClassNotFoundException {
        ...
        CountingBloomFilterMemory<String> cbf = new CountingBloomFilterMemory<>(new FilterBuilder(1000, 0.2).hashFunction(HashMethod.MD5).countingBits(countingBits).complete());

        byte[] byteArray;
        byte[] byteArray2;
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream(); ByteArrayOutputStream bos2 = new ByteArrayOutputStream(); ObjectOutput out = new ObjectOutputStream(bos); ObjectOutput out2 = new ObjectOutputStream(bos2)) {
