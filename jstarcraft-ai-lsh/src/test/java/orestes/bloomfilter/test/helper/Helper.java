...
public class Helper {
    // Jedis automatically translates any address which is known to refer to a local
    // IP to the system's hostname.
    // All Redis servers must listen on all networks (bind 0.0.0.0 in redis conf) to
    // workaround this.
    ...
    public static RedisPool getPool() {
        return RedisPool.builder().host(host).port(port).redisConnections(connections).build();
    }
    ...
    public static <T> BloomFilterRedis<T> createRedisFilter(String name, int m, int k, HashMethod hm, boolean overwrite) {
        return new BloomFilterRedis<>(new FilterBuilder(m, k).hashFunction(hm).redisBacked(true).name(name).redisHost(host).redisPort(port).overwriteIfExists(overwrite).redisConnections(connections).complete());
    }
    ...
    public static <T> BloomFilterRedis<T> createRedisFilter(String name, int n, double p, HashMethod hm, boolean overwrite, int database) {
        return new BloomFilterRedis<>(new FilterBuilder(n, p).hashFunction(hm).redisBacked(true).name(name).redisHost(host).redisPort(port).overwriteIfExists(overwrite).database(database).redisConnections(connections).complete());
    }
    ...
    public static <T> BloomFilterRedis<T> createRedisFilterWithReadSlave(String name, int n, double p, HashMethod hm, boolean overwrite, String slaveName, int slavePort) {
        return new BloomFilterRedis<>(new FilterBuilder(n, p).hashFunction(hm).redisBacked(true).name(name).redisHost(host).redisPort(port).addReadSlave(slaveName, slavePort).overwriteIfExists(overwrite).redisConnections(connections).complete());
    }
    ...
    public static <T> BloomFilterRedis<T> createRedisPoolFilter(String name, int n, double p, HashMethod hm, boolean overwrite, int database) {
        return new BloomFilterRedis<>(new FilterBuilder(n, p).hashFunction(hm).redisBacked(true).name(name).pool(RedisPool.builder().host(host).port(port).database(database).redisConnections(connections).build()).overwriteIfExists(overwrite).complete());
    }
    ...
    public static <T> BloomFilterRedis<T> createRedisSentinelFilter(String name, int n, double p, HashMethod hm, boolean overwrite, int database) {
        return new BloomFilterRedis<>(new FilterBuilder(n, p).hashFunction(hm).redisBacked(true).name(name).pool(RedisPool.sentinelBuilder().master(sentinelClusterName).sentinels(getSentinelNodes()).database(database).redisConnections(connections).build()).overwriteIfExists(overwrite).redisConnections(connections).complete());
    }
    ...
    public static <T> CountingBloomFilterRedis<T> createCountingRedisFilter(String name, int m, int k, HashMethod hm, boolean overwrite) {
        return new CountingBloomFilterRedis<>(new FilterBuilder(m, k).hashFunction(hm).redisBacked(true).name(name).redisHost(host).redisPort(port).overwriteIfExists(overwrite).redisConnections(connections).complete());
    }
    ...
    public static <T> CountingBloomFilterRedis<T> createCountingRedisFilter(String name, int n, double p, HashMethod hm, boolean overwrite, int database) {
        return new CountingBloomFilterRedis<>(new FilterBuilder(n, p).hashFunction(hm).redisBacked(true).name(name).redisHost(host).redisPort(port).overwriteIfExists(overwrite).database(database).redisConnections(connections).complete());
    }
